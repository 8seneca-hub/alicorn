...
Before a cgroup-wide stop, obtain a fresh `alicorn-ide terminal list --json` result using the same OS
...
`sudo -Hu orca /home/orca/.local/bin/alicorn-ide terminal list --json`). Replace both `orca` and
`/home/orca` with the service account and home used by the unit; an extracted deployment may use
its absolute `resources/bin/alicorn-ide` launcher instead. A safe empty census is untruncated, has an explicit `hostScope`, covers every
