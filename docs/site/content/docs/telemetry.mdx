...
- **Always off when:** `DO_NOT_TRACK=1` or `ALICORN_TELEMETRY_DISABLED=1` is set.
...
**`ALICORN_TELEMETRY_DISABLED=1`** — Orca-specific kill switch with the same semantics as
`DO_NOT_TRACK`.
