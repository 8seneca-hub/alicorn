...
*Added 2026-09-06 from two posts Huy and Nghia shared. This note records what the framework says,
where Alicorn already has it, and the four things we adopt because of it.*

## Sources

- Anatoli Kopadze, *Graph Engineering explained: what it is, when to use it and when not to* (X
...
- @hanakoxbt, *Loops and Graphs: how to stop babysitting agents and only approve the last step* (X
...
- **A loop is a check that can fail.** Produce, check, correct, repeat until green. The check is
  programmatic, written first; the loop lives *inside* a unit of work.
- **A graph is the layer above.** Nodes are bounded jobs, edges are dependencies. The graph decides
  ...
  between them.
- **Four kinds of node.** *Splitter* (decomposes; the most consequential design choice), *worker*
  (one unit, isolated context), *code node* (deterministic transformation — merge, rank, dedupe —
  never routed through a model), *gate* (approval).
...
| Splitter | The Foreman lead: writes no code, reads no implementation, decomposes and re-plans | CLAUDE.md *Foreman is an add-on*, FM1–FM3 |
...
| Human at the merge | Merge and deploy gate regardless of track record — *hard stops never retire* | CLAUDE.md invariants |
...
| When not to use a graph | `execution_strategy: single` is the default and stays so; `orchestrated` is opt-in with the meter visible; escalation is offered, never applied | CLAUDE.md *Execution: two axes* |
| Loops | Required checks plus the correction edge inside a stage; the fix loop in the development process | tier 1 item 4, WF2 |
| Prompts → Agents → Loops → Graphs | An ADE *is* the graph layer: the unit of work is a ticket, agents hand work to each other, a human is interrupted only at gates | CLAUDE.md *What we are building* |
...
- **The gate is a property of every hand-off, not a node.** A gate node would let a workflow author
  forget one. Evaluating the policy at every transition is what makes *hard stops never retire*
...
|---|---|---|---|
| **WF5** | `stages.kind = worker \| code`. A **code stage** runs a deterministic command (merge, rank, dedupe, format, generate) with no model and no member; it produces a `step_outcome` like any stage and takes the forward or correction edge on its exit status. | Token efficiency: routing deterministic work through a model is the most common waste the framework names. Today every stage assumes a member. | workflows-stages |
