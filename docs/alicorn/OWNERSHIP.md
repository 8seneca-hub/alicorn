...
vertical slice — the owner does its server _and_ desktop side — so the two of us rarely edit the same
file. Where we must, the rule in _Shared files_ applies. Task keys (`A5`, `C3`, …) are the ones in the
...
`D3` (reviewer ≠ author backend) moved from _Quality gates_ to _Members_ because it lives inside the
...
- **Clients split by service, not by owner**: B2 (Nghia) = control-api client + ledger _reads_
  (`getProvenance`, `getRunCost`); C3 (Huy) = `src/main/alicorn/ledger/ledger-writer.ts` = ledger
  _writes_ (`postStepOutcome`, `patchStepOutcomeSpend`, `postStepVerification`, `postContextCapture`).
...
Keep Plane in sync: move the issue to _In Progress_ when you start and _Done_ when the PR merges.
