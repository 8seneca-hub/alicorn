...
(function () {
  ...
  var NAV = JSON.parse(el.getAttribute('data-nav') || '[]').map(function (t) { return t.id; });
  ...
  function homeOf(frame) { return frame.getAttribute('data-home') || HOME; }
  function navOf(frame) {
    var raw = frame.getAttribute('data-nav');
    if (!raw) return NAV;
    try { return JSON.parse(raw).map(function (t) { return t.id; }); } catch (e) { return NAV; }
  }
  ...
  function show(frame, id, opts) {
    ...
    var nav = navOf(frame);
    var owner = nav.indexOf(id) > -1 ? id : (opts && opts.owner) || id;
    ...
    if (opts && opts.push) { st.push(id); } else { st = [id]; }
    stacks.set(frame, st);
  }

  function sheet(frame, id, open) {
    var w = frame.querySelector('[data-sheetwrap="' + id + '"]');
    if (w) w.setAttribute('data-open', String(open));
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    if (t.hasAttribute('data-sheet')) { sheet(frame, t.getAttribute('data-sheet'), true); return; }
    if (t.hasAttribute('data-sheetclose')) { sheet(frame, t.getAttribute('data-sheetclose'), false); return; }
    if (t.hasAttribute('data-popback')) {
      var st = stacks.get(frame) || [];
      st.pop();
      show(frame, st[st.length - 1] || homeOf(frame));
      return;
    }
    if (t.hasAttribute('data-tab')) { show(frame, t.getAttribute('data-tab')); return; }
    // A pushed screen keeps the owning section highlighted.
    show(frame, t.getAttribute('data-go'), { push: true, owner: t.getAttribute('data-owner') || undefined });
  });

  frames().forEach(function (f) { if (f.querySelector('[data-scr]')) show(f, homeOf(f)); });
})();
