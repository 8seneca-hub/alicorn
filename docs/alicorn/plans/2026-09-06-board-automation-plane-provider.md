...
```
- [ ] **Step 1: Probe (read-only, no commit of the key):** with an API key in `PLANE_API_KEY`, `curl -s -A curl/8.7.1 -H "X-API-Key: $PLANE_API_KEY" https://projects.8seneca.com/api/v1/workspaces/8seneca/projects/` and the states/issues/comments endpoints for one project; record the exact envelope (`results`, `next_cursor`, `next_page_results`, `total_count`), issue fields, state `group` values, and the 429 body in `docs/alicorn/plane-api-notes.md`. Adjust the types above to what the instance returns (Cloudflare requires a browser-like or curl UA; the client sets `User-Agent: alicorn-desktop`).
