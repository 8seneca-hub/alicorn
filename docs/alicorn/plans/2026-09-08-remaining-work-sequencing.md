...
This is an _ordering_ plan, not a specification. Every module already has an implementation plan in
...
wrong on every row. The _seams_ it documents (`alicornFetch`, SQLite v31, client-splits-by-service) are
...
per A9) and export `ALICORN_TEST_POSTGRES_URL`, or every Postgres suite silently _skips_ rather than
...
CLAUDE.md is explicit: merge upstream greedily _until_ the rebrand CI gate goes green, then pin
...
It also sequences the interface work: CLAUDE.md says to do the deep tab-model rewrite _after_ the
...
- **I5** — relay tokens minted by the Control API — _"Depends on: I2, **BC1**"_
- **BC1** — relay on Alicorn infrastructure — _"Depends on: **I5**"_
...
The plan above is a correct _order_. It is not a schedule that fits. Three honest options, and this is a
...
- One short-lived branch per Plane issue, `alc-<n>-<slug>`, PR'd into `main`, rebased before merge.
  Move the issue to _In Progress_ on start and _Done_ on merge.
