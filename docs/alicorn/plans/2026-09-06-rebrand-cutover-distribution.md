...
```
- [ ] **Step 1: Failing tests** — regex matches `orca orchestration send`, `  orca status`, `$(orca …)`, not `orca-ide`, `/usr/local/bin/orca`, `alicorn orca-shim`, `GNOME Orca`; baseline compare flags new file entries and a grown count.
- [ ] **Step 2: Implement**; seed `config/rebrand-cli-baseline.txt` with the current 428 findings (`path:line` per row) so the gate is green today and can only shrink.
- [ ] **Step 3: Verify** `pnpm run verify:rebrand-cli-gate` passes; inject one `orca status` into a skill guide → fails; revert.
- [ ] **Step 4: Commit** `ci: rebrand CLI gate — bare orca invocations may only decrease`.
