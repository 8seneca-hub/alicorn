...
- `config/electron-builder.config.cjs` is the _only_ source of packaging identity — `package.json`
...
- `config/scripts/generate-bundled-skill-guides.mjs` (`--write`/`--check` via
  ...
  _topic names_ themselves are branded, distinct from invocation text inside bodies) and a
...
- 963 distinct `ORCA_*` identifiers (§1). A hook-protocol **version already exists**:
  ...
  **This is the reuse target for R4's "hook version" requirement** — the plumbing exists; renaming
  needs (a) the env-var name changed everywhere referenced, (b) a decision on whether the _value_
...
- Reuse candidate for _how_ to do versioned reinstall: `src/main/ssh/ssh-relay-versioned-install.ts`
...
- `audit-localization-coverage.mjs` (`verify:localization-coverage`) audits _coverage_ (untranslated
  strings), not _staleness_ (translated value whose English source changed) — **no existing script
...
- **Directly reusable for the brand-name half of L1**: `config/scripts/locale-brand-mistranslations.mjs`
  ...
  to flag rows still emitting the _old_ brand's mistranslation forms post-rebrand (proof the English
...
'SignPath Foundation'` (:406) is what electron-updater checks the _installed_ app's
...
- **Windows EDR posture**: do not introduce `-ExecutionPolicy Bypass`, `-EncodedCommand`, or runtime
  `Add-Type` in _new_ code for R2/R4/DS2 without reading `docs/reference/windows-edr-posture.md`
...
- **Git provider compatibility**: nothing GitHub-specific was hardcoded in the CLI's git operations
  ...
  URLs) — if Alicorn ever changes git _hosting_ provider (not just repo owner) this is a bigger
...
3. **Update feed cutover ordering (R2/BC2).** `updater-prerelease-feed.ts` string-matches
   `github.com/stablyai/orca` against _fetched feed content_ (Atom scrape + tag-URL regex), not
...
6. **Stale-translation audit (L1) has no existing script to extend** — `audit-localization-coverage.mjs`
   catches _missing_ translations, `locale-brand-mistranslations.mjs` catches _brand-name_
