...
for (const f of files) {
  ...
  for (const m of html.matchAll(/href="([^"]+)"/g)) {
    ...
    if (path.startsWith('/assets/')) {
      if (!existsSync('dist' + path)) { console.log(`  MISSING ASSET  ${f} → ${href}`); bad++; }
      continue;
    }
    const target = path === '' ? '/' + f.replace('.html','') : path.replace(/\/$/,'') || '/';
    if (!pages.has(target)) { console.log(`  DEAD LINK      ${f} → ${href}`); bad++; continue; }
    if (hash && !(ids[target]?.has(hash))) { console.log(`  DEAD ANCHOR    ${f} → ${href}`); bad++; }
  }
}
