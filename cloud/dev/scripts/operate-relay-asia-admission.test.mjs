...
function harness(initialSelector) {
  ...
  const post = async (url, body) => {
    ...
    if (parsed.pathname === '/v1/admin/cell-status') {
      return {
        status: {
          cellUrl: `https://${body.cellId.split('-').at(-1)}.relay.alicorn.8seneca.com`,
  ...
  const commitWithoutResponse = async (path, body) => {
    await post(`https://relay.alicorn.8seneca.com${path}`, body)
  ...
  return {
    ...
    apply: async (attemptId, membership) => await post(
      'https://relay.alicorn.8seneca.com/v1/admin/admission-selector/apply',
