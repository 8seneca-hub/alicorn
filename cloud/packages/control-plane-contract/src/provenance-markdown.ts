...
// Why per step and not just the counts: "was a human asked?" is only half the trail — the other
// half is whether they agreed with the policy that asked them, and that is per decision.
function renderGateDecisions(view: ProvenanceView): string[] {
  const { gate, auto, unknown } = view.gateCounts
  const parts = [`${gate} gated`, `${auto} automatic`]
  if (unknown > 0) {
    parts.push(`${unknown} unrecorded`)
  }
  const lines = view.steps
    .filter((step) => step.gate.agreement.recorded)
    .map((step) => {
      const agreement = step.gate.agreement
      if (!agreement.recorded) {
        return ''
      }
      const verdict = agreement.agreed ? 'agreed with' : 'overrode'
      const shown = agreement.recommendationShown ? 'shown' : 'not shown'
