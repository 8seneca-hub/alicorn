...
Use `alicorn linear` when Linear is the source of task context or ticket updates. On Linux, use `alicorn-ide` wherever this file says `alicorn`.

`alicorn-linear` and `linear-tickets` are skill names, not CLI namespaces. Always run `alicorn linear ...` commands.
...
If the installed CLI help disagrees with this skill, trust `alicorn linear --help` for the available command surface and tell the user the skill guidance may be stale.
...
```bash
alicorn linear issue --current --full --json
```

Use search when the task names a ticket but the current worktree is not linked:

```bash
alicorn linear search "auth bug" --workspace all --limit 10 --json
alicorn linear issue ENG-123 --full --json
```
...
Do not use `alicorn linear attach` to read screenshots. That command creates link attachments, such as PR/MR links, and does not retrieve inline media files.

## Common Commands

```bash
alicorn linear save-issue [<id>] [--current] [--team <key|id>] [--title <title>] [--description <text> | --body-file <path|->] [--state <state>] [--assignee me|<user>|null] [--priority none|low|medium|high|urgent] [--estimate <number>|null] [--due-date <yyyy-mm-dd>|null] [--label <label>]... [--project <project>|null] [--parent-id <issue>|null] [--write-id <uuid>] [--workspace <id>] [--json]
alicorn linear issue [<id>] [--current] [--comments] [--children] [--depth <n>] [--attachments] [--relations] [--activity] [--full] [--workspace <id>] [--json]
alicorn linear list-issues [--team <team>] [--cycle <cycle>] [--label <label>] [--limit <n>] [--query <text>] [--state <state>] [--cursor <cursor>] [--order-by createdAt|updatedAt] [--project <project>] [--release <release>] [--assignee <user|me|null>] [--delegate <user|me|null>] [--parent-id <issue|null>] [--priority <0-4>] [--created-at <datetime|duration>] [--updated-at <datetime|duration>] [--include-archived] [--workspace <id>|all] [--json]
alicorn linear relation add [<id>] [--current] --related <issue> --type blocks|blocked-by|related|duplicate-of [--workspace <id>] [--json]
alicorn linear relation remove [<id>] [--current] --related <issue> --type blocks|blocked-by|related|duplicate-of [--workspace <id>] [--json]
alicorn linear search <query> [--limit <n>] [--workspace <id>|all] [--json]
alicorn linear team list [--workspace <id>|all] [--json]
alicorn linear team members --team <key|id> [--workspace <id>] [--json]
alicorn linear team states --team <key|id> [--workspace <id>] [--json]
alicorn linear team labels --team <key|id> [--workspace <id>] [--json]
alicorn linear project list [--query <text>] [--limit <n>] [--workspace <id>|all] [--json]
alicorn linear list [--filter assigned|created|all|completed|open] [--team <key|id>] [--limit <n>] [--workspace <id>|all] [--json]
alicorn linear status set [<id>] [--current] --to <state> [--workspace <id>] [--json]
alicorn linear assignee set [<id>] [--current] (--me | --to-id <userId>) [--workspace <id>] [--json]
alicorn linear assignee clear [<id>] [--current] [--workspace <id>] [--json]
alicorn linear priority set [<id>] [--current] --to none|low|medium|high|urgent [--workspace <id>] [--json]
alicorn linear priority clear [<id>] [--current] [--workspace <id>] [--json]
alicorn linear estimate set [<id>] [--current] --to <number> [--workspace <id>] [--json]
alicorn linear estimate clear [<id>] [--current] [--workspace <id>] [--json]
alicorn linear due-date set [<id>] [--current] --to <yyyy-mm-dd> [--workspace <id>] [--json]
alicorn linear due-date clear [<id>] [--current] [--workspace <id>] [--json]
alicorn linear label add [<id>] [--current] --label <labelId-or-exact-name>... [--workspace <id>] [--json]
alicorn linear label remove [<id>] [--current] --label <labelId-or-exact-name>... [--workspace <id>] [--json]
alicorn linear label set [<id>] [--current] --label <labelId-or-exact-name>... [--workspace <id>] [--json]
alicorn linear comment add [<id>] [--current] (--body <text> | --body-file <path|->) [--reply-to <commentId>] [--write-id <uuid>] [--workspace <id>] [--json]
alicorn linear attach [<id>] [--current] --url <url> [--title <title>] [--write-id <uuid>] [--workspace <id>] [--json]
alicorn linear create --title <title> [--body <text> | --body-file <path|->] [--team <key|id>] [--project <projectId-or-exact-name>] [--state <stateId|exact-name>] [--assignee me|<userId>] [--priority none|low|medium|high|urgent] [--estimate <number>] [--due-date <yyyy-mm-dd>] [--label <labelId-or-exact-name>]... [--parent <id> | --parent-current] [--write-id <uuid>] [--workspace <id>] [--json]
```
...
```bash
alicorn linear team list --workspace all --json
alicorn linear team states --team <key-or-id> --workspace <workspaceId> --json
alicorn linear team labels --team <key-or-id> --workspace <workspaceId> --json
alicorn linear team members --team <key-or-id> --workspace <workspaceId> --json
alicorn linear project list --query <project-name> --workspace <workspaceId> --json
```
...
The PR/MR command is `alicorn linear attach`; there is no `attach-pr` command.

Attach the PR/MR link:

```bash
alicorn linear attach --current --url <pr-or-mr-url> --title "PR/MR link" --json
```

Use stdin for multiline comments:

```bash
alicorn linear comment add --current --body-file - --json
```
...
2. Otherwise try `alicorn linear status set --current --to "In Review" --json`.
...
```bash
alicorn linear create --title <title> --parent-current --body-file - --json
```
...
```bash
alicorn linear issue <id> --workspace <workspaceId> --json
```
...
Confirm `alicorn status --json` unless already checked this turn, then read the current issue with `alicorn linear issue --current --full --json`. For completion, attach the PR/MR link, add one completion comment, and move status only when the target state is deterministic and non-regressive.
