...
description: >-
  ...
  waits, task DAGs, decision gates, or coordinator loops. Use `alicorn-cli`
  ...
  for results, or coordinate a DAG. Use `alicorn-cli` for terminal control,
  ...
  `alicorn-cli` for Orca's embedded pages and a page-automation tool such as
...
Use this skill when coordination state matters. For lightweight terminal prompts or basic worktree/terminal/built-in-browser control, use `alicorn-cli`.

## Tool Boundary

If a task says to use Orca orchestration, the coordinator must create or bind a Run, create the Task with `alicorn orchestration task-create`, then attach the worker with either the preferred `alicorn orchestration worker-start` composition or the low-level `alicorn orchestration dispatch --inject` path.
...
```bash
alicorn orchestration task-list --json
alicorn orchestration dispatch-show --task <task_id> --json
```
...
- `alicorn status --json` should show a running runtime.
- `alicorn` must be on PATH (`alicorn-ide` on Linux).
- The orchestration experimental feature must be enabled in Settings > Experimental.
- `alicorn orchestration` commands are RPC calls to the running Orca runtime.
...
```bash
alicorn orchestration run-use --id <adopted_run_id> --takeover-legacy --json
alicorn orchestration check --run <adopted_run_id> --json
```
...
- Do not use `alicorn orchestration dispatch --inject` for full handoffs. It injects a coordinator preamble that tells the worker to send `worker_done`, heartbeat, and `ask` messages, then end its turn under the original terminal's dispatch lifecycle.
- Do not run `alicorn orchestration task-create`, `alicorn orchestration dispatch --inject`, or `alicorn orchestration check --wait` for full handoffs. Do not peek at terminal output after prompt delivery to monitor progress.
...
```bash
alicorn orchestration send --subject <text> [--to <run:id|dispatch:id|legacy_handle>] [--from <handle>] [--body <text>] [--type <type>] [--priority <level>] [--thread-id <id>] [--payload <json>] [--json]
alicorn orchestration check [--terminal <handle>] [--ack <delivery_id>] [--peek|--all] [--types <type,...>] [--format] [--wait] [--timeout-ms <n>] [--json]
alicorn orchestration reply --id <msg_id> --body <text> [--from <handle>] [--json]
alicorn orchestration ask (--question <text>|--resume <msg_id>) [--options <csv>] [--timeout-ms <n>] [--from <handle>] [--json]
alicorn orchestration inbox [--limit <n>] [--json]
```
...
- `alicorn orchestration check --peek --format --json` returns locally formatted unread mail without consuming it; it never writes to terminal input or remotely wakes another terminal. Use `orchestration dispatch --inject` to deliver a tracked task, or `terminal send` when an existing agent needs a free-form prompt.
- While supervising workers manually, use `check --wait --types worker_done,escalation,question --timeout-ms <n>` instead of sleep/poll loops. Process the whole Delivery, reply to `question` messages with `alicorn orchestration reply --id <msg_id> --body <answer> --json`, then acknowledge and keep waiting.
...
```bash
alicorn orchestration run-create --objective <text> --json
alicorn orchestration task-create --spec <text> [--deps <json_array>] [--parent <task_id>] [--json]
alicorn orchestration task-list [--status <status>] [--ready] [--brief] [--json]
alicorn orchestration task-update --id <task_id> --status <status> [--result <json>] [--json]
alicorn orchestration dispatch --task <task_id> --to <handle> [--from <handle>] [--inject] [--json]
alicorn orchestration dispatch-show --task <task_id> [--json]
```
...
- If the target is a bare shell, omit `--inject`, dispatch for tracking if needed, then send the prompt manually with `alicorn terminal send --terminal <handle> --text <prompt> --enter --json`.
...
```bash
alicorn orchestration run-create --objective "<objective>" --json
alicorn orchestration task-create --spec "<worker A task>" --json
alicorn orchestration task-create --spec "<worker B task>" --json
alicorn orchestration worker-start --task <task_a> --worktree current --agent codex --json
alicorn orchestration worker-start --task <task_b> --worktree current --agent claude --json
```
...
```bash
alicorn orchestration worker-start --task <task_id> --worktree current --agent claude --model opus --effort high --json
```
...
```bash
alicorn orchestration worker-start --task <task_id> --worktree new-child --name <name> --agent codex --setup run --json
# Independent/top-level:
alicorn orchestration worker-start --task <task_id> --worktree new-top-level --name <name> --agent codex --setup run --json
```
...
alicorn orchestration worker-start --task <task_id> --on windows --worktree new-top-level --repo <exact_remote_repo_selector> --name <name> --agent codex --setup run --json
alicorn orchestration worker-show --dispatch <dispatch_id> --json
alicorn orchestration worker-read --dispatch <dispatch_id> --limit 50 --json
alicorn orchestration send --to dispatch:<dispatch_id> --subject "Follow-up" --body "<attempt-specific guidance>" --json
```
...
```bash
alicorn orchestration check --wait --types worker_done,escalation,question --timeout-ms 900000 --json
...
alicorn orchestration check --ack <delivery_id> --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```

After processing each accepted `worker_done`, choose the terminal's next owner before you acknowledge the Delivery or wait again. If the same exact agent has an immediate follow-up Task, read the `worker.agent_terminal_handle` field of `worker-show --dispatch <dispatch_id> --json`, then run `alicorn orchestration worker-start --task <next_task_id> --terminal <handle> --json` so Orca transfers cleanup ownership to the new Dispatch. Otherwise run `alicorn orchestration worker-release --dispatch <dispatch_id> --json`.

Run `worker-release` after both succeeded and failed `worker_done` reports unless the user explicitly asked to keep that worker live. Release is post-completion cleanup, not cancellation: Orca first preserves inspectable output, then closes only the exact agent terminal owned by that settled Dispatch. Reused or pre-existing terminals, setup terminals, coordinators, active workers, user-taken-over terminals, and identities Orca cannot prove are retained. If the user explicitly asks to keep the live terminal for debugging, record that exception with `alicorn orchestration worker-retain --dispatch <dispatch_id> --json` instead of silently skipping cleanup. When the user is finished, the same Dispatch can be passed to `worker-release`, which clears the requested retention and releases the terminal.
...
```bash
alicorn orchestration send --type worker_done --subject "<status>" --body "<what changed, findings, and what remains>" --task-id <task_id> --dispatch-id <dispatch_id> --outcome succeeded --files-modified "path/a,path/b" --json
...
```bash
alicorn orchestration gate-create --task <task_id> --question <text> [--options <json_array>] [--evaluate] [--stage-key <key>] [--json]
alicorn orchestration gate-resolve --id <gate_id> --resolution <text> [--json]
alicorn orchestration gate-list [--task <task_id>] [--status <status>] [--json]
alicorn orchestration verify-record --task <task_id> --name <text> --status <passed|failed|skipped|error> [--kind <kind>] [--optional] [--detail <json_object>] [--json]
...
alicorn orchestration policy-set --project <project_id> --mode <always_gate|evidence|never_gate> [--stage-key <key>] [--member <member_id>] [--min-runs <n>] [--min-accept-rate <0..1>] [--max-files <n>] [--max-spend-cents <n>] [--expires-at <iso8601>] [--json]
alicorn orchestration policy-list --project <project_id> [--json]
alicorn orchestration evidence --task <task_id> [--stage-key <key>] [--json]
```
...
Recovery only: `alicorn orchestration reset --tasks|--messages|--all --json` clears the selected local orchestration database state. Do not run it during active coordination unless explicitly abandoning that state.

## Ledger

`alicorn ledger report [--stage <key>] [--project <id>] [--member <id>] [--run <id>] [--strategy <single|orchestrated>] [--since <iso>] [--until <iso>] [--json]` prints `interruptions_per_completed_task` from the Alicorn ledger — how often a human had to intervene per finished task. It prints two denominators side by side: `completedTasks` (tasks with at least one successful step) and `tasksTouched` (any settled outcome, failures included). Read the first; a wide gap between them means steps are failing.
...
```bash
alicorn worktree create --name <task-name> --no-parent --agent codex --prompt "<task brief>" --setup run --json
```
...
```bash
alicorn terminal send --terminal <handle> --text "<task brief>" --enter --json
```

Custom Codex model/effort handoff:

`alicorn worktree create --agent codex --prompt ...` launches the known Codex agent but does not accept Codex-specific `--model` or `-c model_reasoning_effort=...` arguments. When the user asks for a specific Codex model or effort, create the independent worktree first, launch Codex with the requested command in that worktree, wait only for TUI readiness if prompt delivery would otherwise race startup, send the prompt, and stop.
...
Use the exact full `<repo-id>::<path>` worktree id returned by `alicorn worktree create --json`; a bare repo id cannot target the new worktree.

```bash
alicorn worktree create --name <task-name> --no-parent --setup run --json
alicorn terminal create --worktree id:<newFullWorktreeId> --title <task-name> --command 'codex --model gpt-5.5 -c model_reasoning_effort="xhigh"' --json
alicorn terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
alicorn terminal send --terminal <handle> --text "<task brief>" --enter --json
```

Wait only for `tui-idle` when needed to avoid losing the prompt. Do not monitor task completion.

`--no-parent` only controls Orca lineage; it does not choose the Git base. If the work should start from the repo default base, omit `--base-branch` so Orca uses that default, or explicitly pass the repo default base (`origin/main`, `origin/master`, or the `alicorn repo show --repo <selector> --json` value); never base it on the current feature branch unless the user explicitly asks for stacked work or "branch from current". Put current-branch context in the prompt instead.
...
```bash
alicorn terminal create --worktree active --title <task-name> --command "codex" --json
alicorn terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
alicorn orchestration dispatch --task <task_id> --to <handle> --inject --json
```
...
Use `alicorn worktree create --prompt ...` or `alicorn terminal send ...` for full handoffs or untracked/lightweight prompts. Those paths do not attach `taskId`/`dispatchId`; the worker should not send lifecycle messages unless the prompt supplies a live orchestration preamble.
...
```bash
alicorn terminal list [--worktree <selector>] [--include-visual-layouts] [--json]
alicorn terminal create [--worktree <selector>] [--title <text>] [--command <cmd>] [--json]
alicorn terminal split --terminal <handle> [--direction horizontal|vertical] [--command <cmd>] [--json]
alicorn terminal wait --terminal <handle> --for tui-idle --timeout-ms <n> --json
alicorn terminal read --terminal <handle> --json
alicorn terminal send --terminal <handle> --text <text> --enter --json
```

If an older CLI rejects `worktree create --agent`, create the worktree normally, then run `alicorn terminal create --worktree <selector> --command "codex" --json` or `--command "claude"`.
...
- Workers with a valid live preamble must send `worker_done` exactly once from their own terminal with an explicit `--outcome succeeded` or `--outcome failed`:
  `alicorn orchestration send --type worker_done --subject "<short status>" --body "<3-sentence summary: what you did, what you found, what's left>" --task-id <task_id> --dispatch-id <dispatch_id> --outcome succeeded --files-modified "path/a" --report-path "<optional>" --json`
...
- For long tasks, send heartbeat/status only when the preamble asks for it, including both IDs:
  `alicorn orchestration send --type heartbeat --subject "alive" --payload '{"taskId":"<task_id>","dispatchId":"<dispatch_id>","phase":"implementing"}' --json`
...
```bash
alicorn terminal create --worktree active --title login-css-worker --command "claude" --json
alicorn terminal wait --terminal <handle> --for tui-idle --timeout-ms 60000 --json
alicorn orchestration task-create --spec "Fix the login button CSS" --json
alicorn orchestration dispatch --task <task_id> --to <handle> --inject --json
alicorn orchestration check --wait --types worker_done,escalation,question --timeout-ms 900000 --json
```
