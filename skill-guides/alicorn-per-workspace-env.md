---
name: alicorn-per-workspace-env
description: >-
  ...
  provider lifecycle scripts, or resolve an `alicorn vm recipe doctor` failure.
---
...
**The one branch that shapes everything — connection mode:** **Orca-server** (`create` runs `alicorn serve`
...
`environmentRecipes` in `orca.yaml` → `alicorn vm recipe doctor <id> --json` (free) → then the `--provision`
...
2. **Interview the user up front** — gather these choices and confirm them back before scaffolding
   ...
   - **Connection mode:** how Orca attaches to the environment — an **Orca server** (the VM runs
     `alicorn serve` and Orca pairs over its pairing URL; worked example §7f) or **SSH** (Orca connects to
...
8. **Dry-run doctor** — `alicorn vm recipe doctor <recipe-id> --repo-path <repo> --json` (free, static; §9).
   Fix every failure before going live.
9. **[CHECKPOINT] Live self-test** — get the user's OK once, then run
   `alicorn vm recipe doctor <recipe-id> --provision --json` as a loop: it runs create → validates →
...
- **Never snapshot a machine on which the Orca runtime has already run.** The first `alicorn serve` creates
...
This layer inherits §3's rule: if you started `alicorn serve` on the base or auth sandbox to smoke-test it,
...
# 3. remote exec: start alicorn serve in the background and read the recipe JSON it writes (see below)
...
**The exact `alicorn serve` invocation and its output (verified — do not improvise the flags).** Inside the
VM, run:
...
- **`create` does NOT run `alicorn serve` and does NOT emit a `pairingCode`.** Orca itself connects to the
...
# 1. ensure the repo is present and at the right commit on the host (NO alicorn serve here)
...
# 2. print the SSH connection block (NO pairingCode, NO alicorn serve). host/port/username tell Orca's
...
`connection.type:"ssh"` block above instead of starting `alicorn serve`.
...
**Orca-server mode** — boot the env, start `alicorn serve` in it, and print serve's result:
...
Here `pairingCode` (from `alicorn serve --recipe-json`) and `projectRoot` are required; `schemaVersion` (`1`)
and `userData` are optional.

**SSH mode** — do **not** run `alicorn serve`; print the `connection.type:"ssh"` block instead (full shape +
...
Start Orca remotely with `alicorn serve --port "$PORT" --project-root "$ABS_ROOT" --pairing-address
...
`alicorn vm recipe doctor <recipe-id> --repo-path <repo> --json` validates **static wiring only** — it does
...
`alicorn vm recipe doctor <recipe-id> --repo-path <repo> --provision --json` actually runs the recipe end
