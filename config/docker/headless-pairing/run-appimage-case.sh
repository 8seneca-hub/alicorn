...
appimage=${ALICORN_TEST_APPIMAGE:-/artifacts/squashfs-root/AppRun}
timeout_seconds=${ALICORN_STARTUP_TIMEOUT_SECONDS:-12}
pairing_address=${ALICORN_PAIRING_ADDRESS:-127.0.0.1}
port=${ALICORN_SERVE_PORT:-0}
...
app_args=("${launcher[@]}")
if [[ ${ALICORN_TEST_NO_SANDBOX:-1} == 1 ]]; then
  app_args+=(--no-sandbox)
fi
app_args+=(serve --port "$port" --pairing-address "$pairing_address")
if [[ ${ALICORN_READY_JSON:-0} == 1 ]]; then
  app_args+=(--json)
fi
if [[ ${ALICORN_NO_PAIRING:-0} == 1 ]]; then
  app_args+=(--no-pairing)
fi
...
if [[ $is_appimage == 0 ]]; then
  export APPDIR=${ALICORN_TEST_APPDIR:-"$(dirname "$appimage")"}
fi

if [[ ${ALICORN_KEEP_RUNNING:-0} == 1 ]]; then
  exec "${command[@]}"
fi
