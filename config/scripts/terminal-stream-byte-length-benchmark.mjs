...
const ITERATIONS = Number(process.env.ALICORN_BYTE_LENGTH_BENCH_ITERATIONS ?? '61')
let resultChecksum = 0
let validatedPairs = 0

if (!Number.isSafeInteger(ITERATIONS) || ITERATIONS <= 0) {
  throw new Error(
    `ALICORN_BYTE_LENGTH_BENCH_ITERATIONS must be a positive integer, got ${ITERATIONS}`
  )
}
