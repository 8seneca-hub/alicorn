...
const fixtureDir = join(tempRoot, 'Profile space & ^ (paren) %ALICORN_PATH_TRAP% !bang! 한글')
...
const fixtureEnv = {
  ...process.env,
  CLAUDE_CONFIG_DIR: join(fixtureDir, 'config space & ^ (paren) %ALICORN_ENV_LITERAL% !bang! 한글'),
  ALICORN_ARG_TRAP: 'EXPANDED_ARG',
  ALICORN_PATH_TRAP: 'EXPANDED_PATH',
  ALICORN_FIXTURE_CAPTURE: capturePath,
  ALICORN_FIXTURE_PIDS: pidPath,
  ALICORN_FIXTURE_NODE: process.execPath
}
const reportedEnv = {
  ...fixtureEnv,
  CLAUDE_CONFIG_DIR: join(reportedDir, 'config with spaces 한글'),
  ALICORN_FIXTURE_CAPTURE: reportedCapturePath,
  ALICORN_FIXTURE_PIDS: reportedPidPath
}
...
const fixtureSource =
  ...
  `  writeFileSync(process.env.ALICORN_FIXTURE_PIDS, JSON.stringify({ child: process.pid, grandchild: grandchild.pid }))\n` +
  `  setInterval(() => {}, 1000)\n` +
  `} else {\n` +
  `  writeFileSync(process.env.ALICORN_FIXTURE_CAPTURE, JSON.stringify({ argv: process.argv.slice(2), configDir: process.env.CLAUDE_CONFIG_DIR }))\n` +
  `}\n`
const shimSource = '@echo off\r\n"%ALICORN_FIXTURE_NODE%" "%~dp0capture-child.cjs" %*\r\n'
...
try {
  ...
  for (const [name, segment] of Object.entries({
    ...
    percent: 'profile%ALICORN_PATH_TRAP%',
    bang: 'profile!name',
    ...
    const env = {
      ...
      ALICORN_FIXTURE_CAPTURE: captureFile
    }
