...
const cliPath =
  process.env.ALICORN_COMPUTER_SMOKE_CLI_PATH ?? resolve(repoRoot, 'out', 'cli', 'index.js')
...
const preferredApps = (
  requestedApps ??
  process.env.ALICORN_COMPUTER_SMOKE_APPS ??
  ...
  const child = spawnSync(process.execPath, [cliPath, ...cliArgs], {
    ...
    env: {
      ...
      ALICORN_USER_DATA_PATH:
        process.env.ALICORN_COMPUTER_SMOKE_USER_DATA_PATH ?? defaultDevUserDataPath()
    }
  })
  if (child.status !== 0) {
