...
const ITERATIONS = Number(process.env.ALICORN_FRAME_CHUNK_BENCH_ITERATIONS ?? '40')
const GATE_ITERATIONS = Number(process.env.ALICORN_FRAME_GATE_BENCH_ITERATIONS ?? '2000')
const WARMUP = Number(process.env.ALICORN_FRAME_CHUNK_BENCH_WARMUP ?? '8')
const ROUNDS = Number(process.env.ALICORN_FRAME_CHUNK_BENCH_ROUNDS ?? '6')

for (const [name, value] of [
  ['ALICORN_FRAME_CHUNK_BENCH_ITERATIONS', ITERATIONS],
  ['ALICORN_FRAME_GATE_BENCH_ITERATIONS', GATE_ITERATIONS],
  ['ALICORN_FRAME_CHUNK_BENCH_WARMUP', WARMUP],
  ['ALICORN_FRAME_CHUNK_BENCH_ROUNDS', ROUNDS]
]) {
...
if (ROUNDS % 2 !== 0) {
  throw new Error(`ALICORN_FRAME_CHUNK_BENCH_ROUNDS must be even so each arm leads equally`)
}
