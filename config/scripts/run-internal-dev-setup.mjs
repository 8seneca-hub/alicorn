...
// Why: under a Git Bash setup runner, Orca exports ALICORN_WORKTREE_PATH in MSYS form (/c/...), which
...
export function runInternalDevSetup({
  ...
  const setupPath = env.ALICORN_INTERNAL_DEV_SETUP?.trim()
  ...
  spawnOptionalSetup(spawn, setupPath, env.ALICORN_WORKTREE_PATH || cwd, platform, env)
