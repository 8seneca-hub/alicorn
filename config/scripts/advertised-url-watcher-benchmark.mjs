...
const ITERATIONS = Number(process.env.ALICORN_ADVERTISED_URL_BENCH_ITERATIONS ?? '10000')
const ROUNDS = Number(process.env.ALICORN_ADVERTISED_URL_BENCH_ROUNDS ?? '12')
const WARMUP = Number(process.env.ALICORN_ADVERTISED_URL_BENCH_WARMUP ?? '1000')

for (const [name, value] of [
  ['ALICORN_ADVERTISED_URL_BENCH_ITERATIONS', ITERATIONS],
  ['ALICORN_ADVERTISED_URL_BENCH_ROUNDS', ROUNDS],
  ['ALICORN_ADVERTISED_URL_BENCH_WARMUP', WARMUP]
]) {
...
if (ROUNDS % 2 !== 0) {
  throw new Error('ALICORN_ADVERTISED_URL_BENCH_ROUNDS must be even')
}
