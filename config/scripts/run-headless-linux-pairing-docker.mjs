...
async function startAndWait({
  ...
  const args = [
    ...
    '-e',
    'ALICORN_KEEP_RUNNING=1',
    '-e',
    'LIBGL_ALWAYS_SOFTWARE=1',
    '-e',
    `ALICORN_READY_JSON=${mode === 'json' ? '1' : '0'}`,
    '-e',
    `ALICORN_PAIRING_ADDRESS=${address}`,
    '-e',
    `ALICORN_SERVE_PORT=${port}`,
    '-e',
    `ALICORN_TEST_APPIMAGE=${appPath}`,
    ...(noPairing ? ['-e', 'ALICORN_NO_PAIRING=1'] : []),
    '-v',
