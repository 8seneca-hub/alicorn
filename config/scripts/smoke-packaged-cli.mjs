...
try {
  ...
  assert(list.topics.some((topic) => topic.name === 'alicorn-cli'))
  assert.match((await run(['skills', 'get', 'alicorn-cli'])).stdout, /name: alicorn-cli/)
  ...
  const update = JSON.parse(
    (await run(['skills', 'update', '--skill', 'alicorn-cli', '--dry-run', '--json'])).stdout
  )
