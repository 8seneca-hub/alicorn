...
async function main(): Promise<void> {
  const environmentName = process.env.ALICORN_PROBE_ENVIRONMENT_NAME
  if (!environmentName) {
    throw new Error('ALICORN_PROBE_ENVIRONMENT_NAME is required')
  }
  ...
  const cycles = readProbeInteger('ALICORN_PROBE_CYCLES', 10, 100)
  const concurrency = readProbeInteger('ALICORN_PROBE_CONCURRENCY', 25, 200)
  const settleMs = readProbeInteger('ALICORN_PROBE_SETTLE_MS', 250, 5_000)
  const cleanupTimeoutMs = readProbeInteger('ALICORN_PROBE_CLEANUP_TIMEOUT_MS', 10_000, 30_000)
