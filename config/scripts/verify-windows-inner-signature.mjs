...
$signature = Get-AuthenticodeSignature -FilePath $env:ALICORN_WINDOWS_INNER_EXECUTABLE
...
export function parseExpectedSigners(value = process.env.ALICORN_WINDOWS_EXPECTED_SIGNERS) {
...
export function parseExpectedThumbprints(value = process.env.ALICORN_WINDOWS_EXPECTED_THUMBPRINTS) {
...
export function getPowerShellSignatureJson(executablePath, spawnSyncImpl = spawnSync) {
  ...
  const result = spawnSyncImpl(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        ALICORN_WINDOWS_INNER_EXECUTABLE: executablePath
      }
    }
  )
