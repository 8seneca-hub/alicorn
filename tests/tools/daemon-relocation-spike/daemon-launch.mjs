...
// src/main/daemon/daemon-init.ts (detached, ipc channel, ALICORN_USER_DATA_PATH).
...
export function launchDaemonHost(options) {
  ...
  const env = {
    ...
    ALICORN_USER_DATA_PATH: workDir
  }
  ...
  // if run against a build that carries the ALICORN_NODE_PTY_NATIVE_DIR patch.
  if (nodePtyNativeDir) {
    env.ALICORN_NODE_PTY_NATIVE_DIR = nodePtyNativeDir
  }
