  ...
  const child = fork(
    ...
    ],
    {
      ...
      env: {
        ...
        ALICORN_USER_DATA_PATH: runtime.userDataDir
      },
...
export async function spawnGenerationCanary(options: {
  ...
  await waitForCondition(`${label} canary readiness`, () =>
    output.includes(`ALICORN_GENERATION_CANARY_READY ${label} ${nonce}`)
  )
  const match = new RegExp(`ALICORN_GENERATION_CANARY_READY ${label} ${nonce} (\\d+)`).exec(output)
...
export async function pingGenerationCanary(canary: GenerationCanary, nonce: string): Promise<void> {
  ...
  const expected = `ALICORN_GENERATION_CANARY_ACK ${label} ${nonce}`
