...
export async function resolveOrchestrationTerminalHandle(
  ...
  const envHandle = process.env.ALICORN_TERMINAL_HANDLE
  if (envHandle && envHandle.length > 0) {
    if (flagName === 'from' && options.validateEnvHandle) {
      // Why: long-lived shells can retain a stale ALICORN_TERMINAL_HANDLE after remint; don't bake it into coordinator preambles.
...
export function throwNoActiveSenderTerminal(): never {
  throw new RuntimeClientError(
    ...
    'Could not determine the sender terminal for this orchestration command. ' +
      'Pass --from <terminal-handle> or run the command inside a live Alicorn terminal with ALICORN_TERMINAL_HANDLE set.'
  )
}
