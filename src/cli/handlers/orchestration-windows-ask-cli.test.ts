...
const originalCliCommand = process.env.ALICORN_CLI_COMMAND
const originalPackagedLauncher = process.env.ALICORN_WINDOWS_PACKAGED_CLI_LAUNCHER
...
describe('packaged Windows legacy ask protocol', () => {
  beforeEach(() => {
    callMock.mockReset()
    process.env.ALICORN_WINDOWS_PACKAGED_CLI_LAUNCHER = '1'
  ...
  afterEach(() => {
    ...
    restoreEnv('ALICORN_CLI_COMMAND', originalCliCommand)
    restoreEnv('ALICORN_WINDOWS_PACKAGED_CLI_LAUNCHER', originalPackagedLauncher)
  ...
  it.each(['orca', 'orca-ide'] as const)(
    ...
    async (command) => {
      process.env.ALICORN_CLI_COMMAND = command
  ...
  it('resumes the committed question without another exit-75 handoff', async () => {
    process.env.ALICORN_CLI_COMMAND = 'orca'
    callMock.mockResolvedValue({
      result: {
        answer: 'yes',
        messageId: 'msg_question',
