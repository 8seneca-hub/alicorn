...
describe('ledger outbox CLI', () => {
  ...
  it('prints the dead count header and one table row in text mode', async () => {
    const result: OutboxListResult = { rows: [DEAD_ROW], deadCount: 1 }
    const call = vi.fn().mockResolvedValue(envelope(result))
    const log = vi.spyOn(console, 'log').mockImplementation(() => {})
    ...
    expect(lines[1]).toBe(
      'lob_1 | step_outcome | 3 | permanent rejection: 422 unprocessable | permanent rejection: 422 unprocessable'
    )
  })

  it('truncates a long last_error to 60 characters in the table row', async () => {
