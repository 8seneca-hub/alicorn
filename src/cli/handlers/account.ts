 ...
 * GUI flow drives this via a browser Alicorn can't reach on a headless host.
 */
...
async function assertAccountImportSupported({ client }: HandlerContext): Promise<void> {
  ...
  if (!status.result.capabilities?.includes(ACCOUNT_IMPORT_RUNTIME_CAPABILITY)) {
    throw new RuntimeClientError(
      ...
      'The running Alicorn runtime is too old to add accounts from the CLI. Update or restart Alicorn and try again.'
    )
  }
}

/** CLI handlers for `alicorn account add [--agent claude|codex]` and `alicorn account list`. */
export const ACCOUNT_HANDLERS: Record<string, CommandHandler> = {
  'account add': async (ctx) => {
    ...
    rejectAccountRemoteSelectionFlags(ctx, 'alicorn account add')
  ...
  'account list': async (ctx) => {
    rejectAccountRemoteSelectionFlags(ctx, 'alicorn account list')
