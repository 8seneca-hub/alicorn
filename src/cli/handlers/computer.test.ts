...
describe('alicorn computer observation CLI handlers', () => {
  beforeEach(() => {
    vi.restoreAllMocks()
    callMock.mockReset()
  ...
  it('prints targeted permission setup command help', async () => {
    ...
    expect(output).toContain(
      'alicorn computer permissions [--id <accessibility|screenshots>] [--json]'
    )
  ...
  it('opens computer permission setup without resolving a worktree', async () => {
    ...
    const output = vi.mocked(console.log).mock.calls[0][0]
    expect(output).toContain('Opened Alicorn Computer Use permission setup')
    expect(output).toContain('/Applications/Alicorn Computer Use.app')
  })
