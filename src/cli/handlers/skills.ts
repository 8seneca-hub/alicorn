...
/** Agents Alicorn can see on this host, as `skills --agent` keys. */
...
function resolveInstallAgentKeys(flags: Map<string, string | boolean>): string[] {
  ...
  throw new RuntimeClientError(
    ...
    'No coding agent detected on this host, so there is no install target. Pass ' +
      ...
      'writes only the shared .agents/skills directory that Alicorn reads.'
  )
}
...
function formatSkillSelectionHelp(verb: SkillMutationVerb, skillNames: string[]): string {
  return [
    ...
    '',
    `Usage: alicorn skills ${verb} --skill <name> [--skill <name> ...]`,
    `   or: alicorn skills ${verb} --all`
  ].join('\n')
}
...
function createSkillMutationHandler(verb: SkillMutationVerb): CommandHandler {
  return async ({ flags, json }) => {
    ...
    if (process.env.ALICORN_CLI_CWD) {
      throw new RuntimeClientError(
        ...
        `alicorn skills ${verb} writes to the machine that runs it, but this shell forwards ` +
          `alicorn to the Alicorn host. Run the same alicorn skills ${verb} command on the machine ` +
    ...
    if (json) {
      ...
      throw new RuntimeClientError(
        'invalid_argument',
        `alicorn skills ${verb} --json only supports --dry-run. Real ${verb}s stream ` +
