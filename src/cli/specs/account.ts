...
export const ACCOUNT_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['account', 'add'],
    summary: 'Add a managed Claude or Codex account by signing in on this Alicorn host',
    usage: 'alicorn account add [--agent claude|codex] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'agent'],
    notes: [
      'Runs the agent login (`claude login` / `codex login`) in this terminal, then registers the account with the local Alicorn runtime.',
      ...
      '--agent defaults to claude. Requires the Alicorn runtime to be running on this machine.'
    ],
    examples: ['alicorn account add', 'alicorn account add --agent codex']
  },
  {
    path: ['account', 'list'],
    summary: 'List managed Claude and Codex accounts on this Alicorn host',
    usage: 'alicorn account list [--json]',
