...
export const ORCHESTRATION_WORKER_COMMAND_SPECS: CommandSpec[] = [
  {
    ...
    notes: [
      ...
      '--on selects only the worker server; the Run and this command remain on the current Alicorn server.',
  ...
  },
  {
    ...
    usage: 'alicorn orchestration worker-show --dispatch <dispatch_id> [--json]',
    ...
    notes: [
      ...
      'observation.agentWait names a worker parked on a prompt only a human can answer, with the evidence that proved it (hook, prompt-text, or title). Null means Alicorn looked and found no wait. An absent field means it never looked — an older host, an unverifiable worker identity, an unreadable pane, or an agent probe that did not answer in time — and never means the worker is not waiting. A waiting worker is healthy, not failed.'
    ]
  },
  {
    ...
    usage:
      'alicorn orchestration worker-read --dispatch <dispatch_id> [--source <auto|transcript|terminal>] [--cursor <cursor>] [--limit <n>] [--json]',
    ...
    notes: [
      ...
      'A returned cursor is pinned to the exact source; start a fresh read if Alicorn reports source_changed.'
    ]
  },
  {
    ...
    usage:
      'alicorn orchestration worker-stop --dispatch <dispatch_id> [--retry-request <id>] [--json]',
  ...
  },
  {
    ...
    usage:
      'alicorn orchestration worker-abandon --dispatch <dispatch_id> [--retry-request <id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'dispatch', 'retry-request'],
  ...
  },
  {
    ...
    usage:
      'alicorn orchestration worker-release --dispatch <dispatch_id> [--retry-request <id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'dispatch', 'retry-request'],
    notes: [
  ...
  },
  {
    ...
    usage:
      'alicorn orchestration worker-retain --dispatch <dispatch_id> [--retry-request <id>] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'dispatch', 'retry-request'],
    notes: [
  ...
  },
  {
    ...
    usage:
      'alicorn orchestration worker-list [--run <run_id>] [--terminal-state <active|reclaimable|retained|release_pending|release_unknown|released>] [--json]',
