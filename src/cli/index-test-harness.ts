    ...
    constructor(
      ...
      const effectivePairingCode =
        remotePairingCode === undefined
          ? (process.env.ALICORN_PAIRING_CODE ?? process.env.ALICORN_REMOTE_PAIRING)
          : remotePairingCode
      const effectiveEnvironment =
        environmentSelector === undefined ? process.env.ALICORN_ENVIRONMENT : environmentSelector
...
export function useWorktreeAwarenessEnvironment(mocks: WorktreeAwarenessMocks): void {
  ...
  const originalDevCliInvocation = process.env.ALICORN_DEV_CLI_INVOCATION
  const originalPairingCode = process.env.ALICORN_PAIRING_CODE
  const originalRemotePairing = process.env.ALICORN_REMOTE_PAIRING
  const originalEnvironment = process.env.ALICORN_ENVIRONMENT
  ...
  beforeEach(() => {
    ...
    delete process.env.ALICORN_DEV_CLI_INVOCATION
  ...
  afterEach(() => {
    ...
    if (originalTerminalHandle === undefined) {
      ...
      process.env.ALICORN_TERMINAL_HANDLE = originalTerminalHandle
    }
    ...
    if (originalDevCliInvocation === undefined) {
      ...
      process.env.ALICORN_DEV_CLI_INVOCATION = originalDevCliInvocation
    }
    if (originalPairingCode === undefined) {
      delete process.env.ALICORN_PAIRING_CODE
    } else {
      process.env.ALICORN_PAIRING_CODE = originalPairingCode
    }
    if (originalRemotePairing === undefined) {
      delete process.env.ALICORN_REMOTE_PAIRING
    } else {
      process.env.ALICORN_REMOTE_PAIRING = originalRemotePairing
    }
    if (originalEnvironment === undefined) {
      delete process.env.ALICORN_ENVIRONMENT
    } else {
      process.env.ALICORN_ENVIRONMENT = originalEnvironment
    }
