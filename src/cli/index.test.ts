...
describe('unknown command surfaces a suggestion', () => {
  ...
  it('prints did-you-mean for a near-miss command and exits non-zero', async () => {
    ...
    expect(stderr).toContain('alicorn worktree')
  })
...
describe('unknown help command surfaces a suggestion', () => {
  it.each([
    ...
    expect(process.exitCode).toBe(1)
    expect(logSpy.mock.calls.flat().join('\n')).toContain('Did you mean: alicorn worktree')
    logSpy.mockRestore()
    process.exitCode = 0
  })
})

describe('alicorn root help', () => {
  ...
  it('progressively discloses Linear commands', async () => {
    ...
    expect(groupHelp).toContain('alicorn linear')
    ...
    expect(issueHelp).toContain('alicorn linear issue [<id>]')
    ...
    expect(searchHelp).toContain('alicorn linear search <query>')
