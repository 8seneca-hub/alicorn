...
describe('orchestration mutation recovery', () => {
  it('queries a known dispatch before issuing the keyed retry', () => {
    ...
    expect((result.data as { nextSteps?: string[] }).nextSteps).toEqual([
      ...
      'After inspecting the Dispatch, if keyed recovery is still needed, run orca orchestration worker-start --task task_1 --retry-request request_1. --retry-request reuses the same operation identity so Alicorn can replay, join, or safely recover it without starting a separate duplicate.'
    ])
  })
