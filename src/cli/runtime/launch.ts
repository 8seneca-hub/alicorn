...
export function launchOrcaApp(): void {
  ...
  throw new RuntimeClientError(
    'runtime_open_failed',
    'Could not determine how to launch Alicorn. Start Alicorn manually and try again.'
  )
}
    ...
    const onClose = (code: number | null, signal: NodeJS.Signals | null): void => {
      ...
      finish(
        new RuntimeClientError(
          ...
          typeof code === 'number'
            ? `Alicorn serve exited before printing valid recipe JSON with code ${code}.`
            : `Alicorn serve exited before printing valid recipe JSON via ${signal}.`
        )
      )
    }
...
function resolveAppRoot(): string {
  ...
  // process.cwd(). Pin it to the app root so `alicorn serve` behaves the same
...
function resolveForegroundOrcaExecutable(): string {
  ...
  throw new RuntimeClientError(
    'runtime_serve_failed',
    'Could not determine how to start Alicorn server. Set ALICORN_APP_EXECUTABLE to the Alicorn executable.'
  )
}
