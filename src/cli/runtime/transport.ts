  ...
  return await new Promise((resolve, reject) => {
    ...
    if (!transport) {
      reject(
        new RuntimeClientError(
          'runtime_unavailable',
          'No compatible transport found in Alicorn runtime metadata.'
        )
      )
      return
    }
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new RuntimeClientError(
          'runtime_timeout',
          'Timed out waiting for the Alicorn runtime to respond.'
        )
      )
    }, timeoutMs)
    ...
    socket.once('error', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'Could not connect to the running Alicorn app. Restart Alicorn and try again.'
        )
      })
    })
    ...
    socket.once('close', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'The Alicorn runtime closed the connection before responding. Restart Alicorn and try again.'
        )
      })
    })
    socket.on('data', (chunk: string) => {
      ...
      while (newlineIndex !== -1 && !settled) {
        ...
        try {
          ...
          finish({
            ...
            error: new RuntimeClientError(
              ...
              'The Alicorn runtime returned an invalid response frame.'
            )
          })
          return
        }
        ...
        // setTimeout().refresh() is stable since Node 10 (Alicorn ships on
        ...
        // the decode boundary so version skew between the CLI and the Alicorn
        ...
        if (response.id !== requestId) {
          finish({
            ...
            error: new RuntimeClientError(
              'invalid_runtime_response',
              'The Alicorn runtime returned a mismatched response id.'
            )
          })
          return
        }
        if (response._meta?.runtimeId && response._meta.runtimeId !== metadata.runtimeId) {
          finish({
            ...
            error: new RuntimeClientError(
              'runtime_unavailable',
              'The Alicorn runtime changed while the request was in flight. Retry the command.'
            )
          })
          return
        }
