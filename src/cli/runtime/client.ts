...
export class RuntimeClient {
  ...
  private async checkOrchestrationContractCompatibility(timeoutMs: number): Promise<void> {
    ...
    if (!response.result.capabilities?.includes(ORCHESTRATION_CONTRACT_RUNTIME_CAPABILITY)) {
      throw new RuntimeClientError(
        ...
        'The connected Alicorn runtime does not support the current orchestration contract. No effects were applied.',
  ...
  async openOrca(timeoutMs = 15_000): Promise<RuntimeRpcSuccess<CliStatusResult>> {
    ...
    throw new RuntimeClientError(
      ...
      'Timed out waiting for an Alicorn desktop window. The runtime may still be running headlessly.'
    )
  }
}
...
function throwDesktopActivationBlocked(): never {
  throw new RuntimeClientError(
    ...
    'Alicorn is running headlessly, but it cannot open a desktop window safely because the persistent terminal provider is unavailable. Quit Alicorn normally and start the app again; do not use open -n.'
  )
}
