...
export function formatCommandHelp(spec: CommandSpec): string {
  const lines = [`alicorn ${spec.path.join(' ')}`, '', `Usage: ${spec.usage}`, '', spec.summary]
...
export function formatGroupHelp(specs: CommandSpec[], group: string): string {
  ...
  const lines = [
    `alicorn ${group}`,
    '',
    `Usage: alicorn ${group} <command> [options]`,
    '',
    'Commands:'
  ]
  ...
  lines.push('', `Run \`alicorn ${group} <command> --help\` for command-specific usage.`)
  return lines.join('\n')
}
...
export function formatFlagHelp(flag: string): string {
  const helpByFlag: Record<string, string> = {
    ...
    comment: '--comment <text>       Comment stored in Alicorn metadata',
    ...
    activate: '--activate             Reveal the new worktree in the Alicorn app',
    ...
    'display-name': '--display-name <name>  Override the Alicorn display name',
    ...
    focus: '--focus                Reveal the created terminal session in Alicorn',
    ...
    page: '--page <id>            Stable browser page id from `alicorn tab list --json`',
  ...
  if (flag === 'current') {
    return '--current              Use the current Alicorn worktree linked Linear issue'
  }
  if (flag === 'comments') {
