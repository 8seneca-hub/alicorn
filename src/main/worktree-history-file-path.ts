/**
 * Recognises a `HISTFILE` value Alicorn itself minted for a worktree.
 *
 * Why: HISTFILE is EXPORTED into the pane, so an Alicorn launched from an Alicorn
 ...
 * only a value Alicorn can prove it minted is dropped, so a HISTFILE the user set
 ...
 * is dropped from ONE spawn's env, so the pane gets Alicorn's own worktree history
...
// shape is the user's, not Alicorn's.
...
/** Drop a `HISTFILE` this process inherited from an outer Alicorn pane. */
