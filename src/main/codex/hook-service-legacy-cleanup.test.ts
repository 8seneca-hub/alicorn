...
describe('CodexHookService', () => {
  it('removes legacy Alicorn-managed hooks from system ~/.codex during install', async () => {
  ...
  it('removes very large legacy Alicorn-managed hook lists from system ~/.codex', async () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    const legacyCommand = legacyManagedHookCommand()
    mkdirSync(systemCodexHome, { recursive: true })
  ...
  it('removes the legacy Alicorn Codex profile file when it only contains managed hooks', async () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
  ...
  it('removes only the legacy Alicorn block from a user-edited Codex profile file', async () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
    const profilePath = join(systemCodexHome, 'orca-agent-status.config.toml')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
