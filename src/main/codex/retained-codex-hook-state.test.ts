...
describe('retained Codex hook state', () => {
  it('repairs Alicorn hooks before a retained shell can launch Codex', async () => {
  ...
  it('removes only Alicorn hooks from retained homes when hooks are disabled', async () => {
    const install = vi.fn(() => status('installed'))
    const refreshRuntimeUserHooks = vi.fn(() => status('not_installed'))
