...
// stamped `complete` on first app-server startup), so rollouts that Alicorn's
...
// Codex's DB-driven surfaces. Alicorn never writes Codex's sqlite schema itself.
...
function buildNativeHealInvocation(
  ...
  return {
    ...
    args: spawnArgs,
    cliPath: command,
    // Why: pin the real home explicitly — nested Alicorn launches can inherit a
