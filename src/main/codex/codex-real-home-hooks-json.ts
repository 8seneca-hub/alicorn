...
export function assertHooksJsonGeneration(
  ...
  if (currentRaw !== expectedRaw || resolveHooksJsonWritePath(hooksJsonPath) !== hooksWritePath) {
    ...
    throw new Error('Codex hooks.json changed while Alicorn prepared its trust repair')
  }
}

/** One-time pristine copy of the user's file, kept under Alicorn's userData. */
...
/** Places Alicorn's managed hook in `definitions`, reusing its existing slot when
...
export function reconcileManagedHookDefinition(
  ...
  if (!hasManagedDirectCommand && nestedLocations.length === 1) {
    ...
    if (definition.matcher === undefined && !hasDirectCommand) {
      ...
      // Why: users can append groups or handlers after Alicorn's first install.
