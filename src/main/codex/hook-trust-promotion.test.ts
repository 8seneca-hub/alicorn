...
describe('codex hook trust write-back promotion', () => {
  ...
  it('keeps an in-Alicorn approval of a user hook across launches and promotes it to ~/.codex', async () => {
    writeSystemUserHook()
  ...
  it('never promotes trust for the Alicorn-managed status hook into ~/.codex', async () => {
    writeSystemUserHook()
    const service = new CodexHookService()
    await service.install()
  ...
  it('promotes an in-Alicorn disable of a mirrored user hook back to the system config', async () => {
    writeSystemUserHook()
    const systemTomlPath = join(systemCodexDir(), 'config.toml')
    writeFileSync(systemTomlPath, upsertHookTrustEntriesInContent('', [systemUserStopEntry()]))
