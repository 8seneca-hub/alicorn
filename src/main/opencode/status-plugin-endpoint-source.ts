export function getStatusPluginEndpointSource(): string[] {
  return [
    ...
    "// OpenCode's Node process (not Alicorn's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on Alicorn restart',
    ...
    '      // writeEndpointFile on the Alicorn side) allocates a fresh inode on',
    ...
    '        // digits in the key for forward-compat with future ALICORN_AGENT_HOOK_*',
    ...
    '  // frozen when OpenCode was fork()ed — stale after an Alicorn restart. The',
    '  // file is rewritten on every Alicorn start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / Alicorn',
