...
import {
  ALICORN_CLOUD_CALLBACK_RESPONSE_HEADERS,
  ALICORN_CLOUD_CALLBACK_SUCCESS_PAGE
...
export function beginOrcaCloudPkceFlow(
  ...
  return new Promise((resolve, reject) => {
    ...
    function writeInvalidCallback(response: ServerResponse): void {
      ...
      response.end('Invalid Alicorn sign-in response.')
    }
    ...
    const server = createServer((request, response) => {
      try {
        ...
        if (url.searchParams.has('error')) {
          response.writeHead(400)
          response.end('Alicorn sign-in was cancelled.')
        ...
        response.writeHead(200, ALICORN_CLOUD_CALLBACK_RESPONSE_HEADERS)
        response.end(ALICORN_CLOUD_CALLBACK_SUCCESS_PAGE)
        resolveFlow(code)
      } catch (error) {
