...
function devSelf(): OrcaOrgMember {
  return {
    ...
    displayName: cleanEnvString(process.env.ALICORN_CLOUD_DEV_DISPLAY_NAME, 'Alicorn Dev'),
    role: 'owner'
  }
}
...
function seedDevRoster(): DevOrgRoster {
  return {
    members: [
      ...
      // Why: userId null exercises the "hasn't signed in to Alicorn yet" disabled row.
