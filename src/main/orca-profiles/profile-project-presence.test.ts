...
describe('profile project presence', () => {
  ...
  it('finds matching projects in other profiles while excluding the active profile', async () => {
    writeProfileState('personal', [
      makeRepo({ id: 'personal-repo', path: 'C:\\Code\\Alicorn', displayName: 'Personal Alicorn' })
    ])
    writeProfileState('work', [
      makeRepo({ id: 'work-repo', path: 'C:\\Code\\Alicorn', displayName: 'Work Alicorn' })
    ])
  ...
  it('keeps SSH projects separate from local projects with the same path', async () => {
    writeProfileState('personal', [
      makeRepo({ id: 'local-repo', path: '/srv/orca', displayName: 'Local Alicorn' })
    ])
    writeProfileState('work', [
      makeRepo({
        id: 'ssh-repo',
        path: '/srv/orca',
        displayName: 'SSH Alicorn',
