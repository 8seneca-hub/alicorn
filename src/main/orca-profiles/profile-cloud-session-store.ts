...
import { getSecretStore, hasSecretStore } from '../../shared/secret-store'
...
export function saveOrcaCloudSession(
  ...
  if (hasSecretStore() && getSecretStore().isEncryptionAvailable()) {
  ...
  // Why: Alicorn account refresh tokens must not silently fall back to plaintext
...
export function readOrcaCloudSession(
  ...
  try {
    ...
    return {
      status: 'decrypt-failed',
      persistence: 'none',
      error: 'Could not decrypt saved Alicorn account session.'
    }
  }
}
