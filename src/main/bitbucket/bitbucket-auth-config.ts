...
export function getEnvAuthConfig(): BitbucketAuthConfig {
  return {
    baseUrl: envValue('ALICORN_BITBUCKET_API_BASE_URL') ?? DEFAULT_API_BASE_URL,
    accessToken: envValue('ALICORN_BITBUCKET_ACCESS_TOKEN'),
    email: envValue('ALICORN_BITBUCKET_EMAIL'),
    apiToken: envValue('ALICORN_BITBUCKET_API_TOKEN')
  }
}
