...
describe('prepareLocalCommitMessageAgentEnv', () => {
  ...
  it('sets CODEX_HOME for host managed Codex accounts', async () => {
    const result = await prepareLocalCommitMessageAgentEnv('codex', {
      prepareForCodexLaunch: () =>
        'C:\\Users\\tester\\AppData\\Roaming\\Alicorn\\codex-accounts\\a\\home'
    })

    expect(result).toEqual({
      ok: true,
      env: expect.objectContaining({
        CODEX_HOME: 'C:\\Users\\tester\\AppData\\Roaming\\Alicorn\\codex-accounts\\a\\home'
      })
    })
  })
  ...
  it('strips a nested-Alicorn CODEX_HOME override when the launch resolves to the real home', async () => {
