...
// Why: kept in sync with pty/wsl-alicorn-env.ts, which path-translates the same keys for WSL.
// ALICORN_WORKSPACE_NAME is a display name and the credential-guard policy is an enum — never paths.
...
export function getSetupEnvVars(repo: Repo, worktreePath: string): Record<string, string> {
  return {
    ALICORN_ROOT_PATH: repo.path,
    ALICORN_WORKTREE_PATH: worktreePath,
    ALICORN_WORKSPACE_NAME: getRuntimePathBasename(worktreePath),
