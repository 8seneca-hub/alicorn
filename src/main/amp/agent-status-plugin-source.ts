...
export const AMP_PLUGIN_MARKER = 'Managed by Alicorn. Do not edit; changes may be overwritten.'
...
export function getAmpPluginSource(): string {
  return [
    ...
    '  const endpointPath = process.env.ALICORN_AGENT_HOOK_ENDPOINT',
    ...
    "      if (match[1] === 'ALICORN_AGENT_HOOK_PORT') out.port = value",
    "      if (match[1] === 'ALICORN_AGENT_HOOK_TOKEN') out.token = value",
    "      if (match[1] === 'ALICORN_AGENT_HOOK_ENV') out.env = value",
    "      if (match[1] === 'ALICORN_AGENT_HOOK_VERSION') out.version = value",
    '    }',
    ...
    '  // Why: Amp sessions can outlive an Alicorn restart; the endpoint file is',
    '  // rewritten on each start, so read it per event before falling back to env.',
    '  const fileEnv = readEndpointFile() ?? {}',
    '  return {',
    '    port: fileEnv.port || process.env.ALICORN_AGENT_HOOK_PORT,',
    '    token: fileEnv.token || process.env.ALICORN_AGENT_HOOK_TOKEN,',
    '    env: fileEnv.env || process.env.ALICORN_AGENT_HOOK_ENV || "",',
    '    version: fileEnv.version || process.env.ALICORN_AGENT_HOOK_VERSION || ""',
    '  }',
    '}',
    '',
    ...
    '    })',
    '  } catch {',
    '    // Why: Alicorn status reporting must never affect the Amp run.',
    '  } finally {',
    ...
    '  // payload closures when Alicorn is down and each POST waits for timeout.',
