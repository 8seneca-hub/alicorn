...
const CURSOR_HOOK_RESPONSE_ENV = 'ALICORN_CURSOR_HOOK_RESPONSE'
...
export function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  if (target === 'local' && process.platform === 'win32') {
    return [
      ...
      // Why: source current endpoint coordinates for PTYs surviving an Alicorn restart.
  ...
  return [
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${ALICORN_AGENT_HOOK_PORT}/hook/cursor" \\',
