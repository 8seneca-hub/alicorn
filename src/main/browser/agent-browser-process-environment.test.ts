...
describe('agent-browser process environment', () => {
  ...
  // Why ownsSocketDirectory is false for both: a directory Alicorn did not derive can be shared with
  // another Alicorn profile, so `session list` under it is no proof of ownership.
  ...
  // Why: the only daemon bound that survives a SIGKILL'd Alicorn, so it must be set on every platform.
