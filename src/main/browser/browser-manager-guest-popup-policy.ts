...
export abstract class BrowserManagerGuestPopupPolicy extends BrowserManagerNavigation {
  protected installGuestPopupPolicy(
    ...
    guest.setWindowOpenHandler(({ url, frameName, disposition, features }) => {
      ...
      // Why: an unnamed, featureless window.open() is Chromium's own new-tab shape, so an Alicorn tab is
      ...
      if (browserTabId && canOpenAsChild) {
        ...
        return {
          ...
          // Why: default child windows lack an address bar; host in an Alicorn origin-bar window so the destination is verifiable.
