...
function getAuthConfig(): AzureDevOpsCreateAuthConfig {
  return {
    pat: envValue('ALICORN_AZURE_DEVOPS_TOKEN') ?? envValue('ALICORN_AZURE_DEVOPS_PAT'),
    accessToken: envValue('ALICORN_AZURE_DEVOPS_ACCESS_TOKEN'),
    username: envValue('ALICORN_AZURE_DEVOPS_USERNAME')
  }
}
...
function classifyCreateError(error: unknown): CreateHostedReviewResult {
  ...
  if (
    ...
    return {
      ...
      code: 'auth_required',
      error:
        'Create PR failed: Azure DevOps is not authenticated. Next step: set ALICORN_AZURE_DEVOPS_TOKEN in this environment.'
    }
  }
