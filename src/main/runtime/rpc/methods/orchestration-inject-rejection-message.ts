...
// Not filtered by `disabledTuiAgents` — that gates Alicorn's launchers, not detection, so a hand-started disabled agent still injects.
...
export function buildInjectRejectionMessage(terminal: string): string {
  return (
    ...
    `Alicorn detects these agent CLIs (${RECOGNIZED_AGENT_PROCESS_NAMES.join(', ')}). ` +
