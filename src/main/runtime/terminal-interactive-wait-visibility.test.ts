...
// Captured verbatim from cursor-agent 2026.08.11-e8db854 driven through Alicorn.
...
// cursor-agent renders a braille spinner in its OSC title while it works, and Alicorn reads
...
describe('terminal interactive-wait visibility (STA-4513, STA-3714)', () => {
  ...
  describe('prompts the runtime already matched but never surfaced', () => {
    it('surfaces a startup trust screen on the pane, not only on terminal wait', async () => {
      // A pane on its trust screen still wears Alicorn's tab title; the agent has set none.
      const { runtime, handle } = await createPane({
  ...
  it('still reports a live dialog restored from terminal history', async () => {
    // Why this case exists: a lane parked on a prompt emits no bytes, so an Alicorn restart is
