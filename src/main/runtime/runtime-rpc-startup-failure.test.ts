...
describe('runtime RPC startup failure reporting', () => {
  ...
  it('shows the CLI impact and local cause', async () => {
    ...
    expect(showMessageBoxMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        type: 'error',
        title: 'Alicorn CLI unavailable',
        message: "Alicorn couldn't start its local command transport.",
  ...
  it.each([
    ['EACCES', "Check permissions on Alicorn's data folder"],
    ['EPERM', "Check permissions on Alicorn's data folder"],
    ...
    ['ENOENT', "Alicorn's data folder may be missing"],
  ...
  it('falls back to a plain restart when the cause is unrecognised', async () => {
    ...
    const detail = showMessageBoxMock.mock.calls[0]?.[1]?.detail as string
    expect(detail).toContain('Restart Alicorn to try again.')
    expect(detail).not.toContain("Check permissions on Alicorn's data folder")
  })
