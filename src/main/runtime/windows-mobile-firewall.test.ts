...
function environment(
  ...
  return {
    platform: 'win32',
    isPackaged: true,
    executablePath: "C:\\Users\\O'Brien\\Alicorn\\Alicorn.exe",
...
describe('windows mobile firewall', () => {
  it('inspects the exact executable, port, and selected interface profile', async () => {
    ...
    expect(script).toContain(
      "Get-NetFirewallApplicationFilter -PolicyStore ActiveStore -Program 'C:\\Users\\O''Brien\\Alicorn\\Alicorn.exe'"
    )
    ...
    expect(script).toContain("C:\\Users\\O''Brien\\Alicorn\\Alicorn.exe")
  ...
  it('repairs only Alicorn mobile pairing on private networks after elevation', async () => {
    ...
    expect(repairScript).toContain("-Name 'Alicorn.MobilePairing'")
    ...
    expect(repairScript).toContain("-Program 'C:\\Users\\O''Brien\\Alicorn\\Alicorn.exe'")
