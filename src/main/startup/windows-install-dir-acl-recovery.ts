  ...
  const started = repairWindowsInstallDirPackageAcl({
    ...
    onDone: (result) => {
      ...
      if (result.mode === 'repaired' && !claimDisproved) {
        ...
        // Never a user-confirmed marker: "keep safe graphics" is a choice, not Alicorn's latch.
...
const CAUSE =
  "Windows permissions on Alicorn's install folder are blocking its own sandboxed processes from reading the files it shipped with."
...
export function describeInstallDirAclPoison(): InstallDirAclPoisonDiagnosis | null {
  ...
  const status =
    poison.stage === 'pending'
      ? 'Alicorn is repairing the permissions now.'
      : 'Alicorn could not repair them, which usually means the folder needs an administrator.'
  return {
    detail: `${CAUSE} ${status}\n\nRun these in an Administrator Command Prompt, then relaunch Alicorn:\n\n${commands.join('\n')}`,
    commands
  }
}
