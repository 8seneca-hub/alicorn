...
const REDIRECT_ATTEMPT_ENV = 'ALICORN_CLI_LAUNCH_REDIRECTED'
...
export function maybeRedirectCliLaunch(
  ...
  if (env[REDIRECT_ATTEMPT_ENV] === '1') {
    process.stderr.write('Unable to start the Alicorn CLI through Electron node mode.\n')
    return { redirected: true, status: 1 }
  }
  if (!exists(cliEntryPath)) {
    process.stderr.write(`Unable to locate the Alicorn CLI entrypoint at ${cliEntryPath}\n`)
    return { redirected: true, status: 1 }
  }
...
function buildElectronRunAsNodeEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  childEnv.ALICORN_NODE_OPTIONS = env.NODE_OPTIONS ?? ''
  childEnv.ALICORN_NODE_REPL_EXTERNAL_MODULE = env.NODE_REPL_EXTERNAL_MODULE ?? ''
