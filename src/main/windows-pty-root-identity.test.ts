...
describe('classifyWindowsTreeKillTarget', () => {
  it('accepts a ConPTY shell spawned directly by this process', () => {
    const rows = [...SYSTEM_CHAIN, link(ALICORN_PID, 900), link(4242, ALICORN_PID)]
  ...
  it('rejects a recycled pid whose ancestry never reaches this process', () => {
    const rows = [...SYSTEM_CHAIN, link(ALICORN_PID, 900), link(4242, 900)]
  ...
  it('reports an exited root as absent rather than sweeping a stale pid', () => {
    const rows = [...SYSTEM_CHAIN, link(ALICORN_PID, 900)]
    expect(classifyWindowsTreeKillTarget(4242, rows, ALICORN_PID)).toBe('absent')
  })
  ...
  it('rejects a recycled pid whose parent has itself already exited', () => {
    ...
    const rows = [link(ALICORN_PID, 900), link(4242, 31337)]
    expect(classifyWindowsTreeKillTarget(4242, rows, ALICORN_PID)).toBe('foreign')
  })
  ...
  it('never force-kills this process tree when the root pid is our own pid', () => {
    const rows = [link(ALICORN_PID, 900)]
    expect(classifyWindowsTreeKillTarget(ALICORN_PID, rows, ALICORN_PID)).toBe('foreign')
  })

  it.each([0, -1, 1.5, Number.NaN])('rejects the invalid root pid %s', (rootPid) => {
    expect(classifyWindowsTreeKillTarget(rootPid, [link(4242, ALICORN_PID)], ALICORN_PID)).toBe(
      'foreign'
    )
  })

  it('treats duplicate rows for the root as unknown, not as ownership', () => {
    const rows = [link(4242, ALICORN_PID), link(4242, 900)]
    expect(classifyWindowsTreeKillTarget(4242, rows, ALICORN_PID)).toBe('unknown')
  })
...
describe('verifyWindowsTreeKillTarget', () => {
  it('classifies a live win32 root against the fresh process table', async () => {
    const readRows = vi.fn().mockResolvedValue([link(4242, ALICORN_PID)])
    await expect(
      verifyWindowsTreeKillTarget(4242, { readRows, ownerPid: ALICORN_PID, platform: 'win32' })
    ).resolves.toBe('own')
  ...
  it('skips the probe off Windows so POSIX teardown keeps its own guards', async () => {
    const readRows = vi.fn()
    await expect(
      verifyWindowsTreeKillTarget(4242, { readRows, ownerPid: ALICORN_PID, platform: 'darwin' })
    ).resolves.toBe('unknown')
...
describe('verifyWindowsTreeKillTarget scan volume', () => {
  const NATIVE_ROWS = [
    { pid: ALICORN_PID, ppid: 900, name: 'orca.exe', commandLine: 'orca.exe' },
    { pid: 4242, ppid: ALICORN_PID, name: 'pwsh.exe', commandLine: 'pwsh.exe' }
  ]
  ...
  it('collapses a 32-wide teardown burst into a single process-table scan', async () => {
    const verdicts = await Promise.all(
      Array.from({ length: 32 }, () =>
        verifyWindowsTreeKillTarget(4242, { ownerPid: ALICORN_PID, platform: 'win32' })
      )
    )
