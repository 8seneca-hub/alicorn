...
vi.mock('../../resources/app-icons/alicorn-watercolor.png?asset', () => ({
  default: 'watercolor-icon'
}))

vi.mock('../../resources/app-icons/alicorn-watercolor.png?asset&asarUnpack', () => ({
  default: 'watercolor-icon-unpacked'
}))

vi.mock('../../resources/app-icons/alicorn-blue.png?asset', () => ({
  default: 'blue-icon'
}))

vi.mock('../../resources/app-icons/alicorn-blue.png?asset&asarUnpack', () => ({
...
describe('app icon selection', () => {
  ...
  it('persists a custom macOS dock icon to the app bundle for inactive Dock pins', async () => {
    ...
    persistMacDockIcon('watercolor', {
      appBundlePath: '/Applications/Alicorn.app',
      execFile,
      isDevApp: false,
      platform: 'darwin'
    })
    ...
    expect(execFile).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          ALICORN_APP_BUNDLE_PATH: '/Applications/Alicorn.app',
  ...
  it('clears the AppKit icon and Finder metadata when switching macOS back to classic', async () => {
    ...
    expect(execFile).toHaveBeenCalledWith(
      '/usr/bin/xattr',
      ['-d', 'com.apple.FinderInfo', '/Applications/Alicorn.app'],
      expect.objectContaining({
        timeout: 10_000
      }),
      expect.any(Function)
    ...
    expect(execFile).toHaveBeenCalledWith(
      '/usr/bin/xattr',
      ['-d', 'com.apple.ResourceFork', '/Applications/Alicorn.app'],
      expect.objectContaining({
        timeout: 10_000
      }),
      expect.any(Function)
