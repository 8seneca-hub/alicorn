...
describe('preflight', () => {
  ...
  it('does not report Claude Agent Teams when only the Alicorn shim is present', async () => {
    execFileAsyncMock.mockImplementation(async (command, args) => {
      if (command !== 'which') {
        throw new Error(`unexpected command ${String(command)}`)
      }
      if (String(args[0]) === 'orca') {
        return {
          environmentResolved: true,
          code: 0,
          stdout: '/Applications/Alicorn.app/Contents/MacOS/orca\n',
          stderr: '',
          timedOut: false
        }
      }
  ...
  it('reports Claude Agent Teams when both Alicorn and Claude are present', async () => {
    execFileAsyncMock.mockImplementation(async (command, args) => {
      if (command !== 'which') {
        throw new Error(`unexpected command ${String(command)}`)
      }
