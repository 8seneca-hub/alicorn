...
beforeEach(async () => {
  ...
  await git(['config', 'user.name', 'Alicorn Test'], repoPath)
  ...
  await git(['config', 'user.name', 'Alicorn Test'], forkPath)
...
describe('reconcileOrphanedPrRemotesWithExec against the real Git binary', () => {
  ...
  it('reclaims a remote orphaned by a worktree removed outside Alicorn (path 3)', async () => {
    ...
    // Simulate a plain `git worktree remove` the user ran outside Alicorn: Alicorn's metadata for
