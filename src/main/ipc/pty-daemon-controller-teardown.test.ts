...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('strips ALICORN_PANE_KEY/TAB_ID/WORKTREE_ID from SSH spawn env when remote agent hooks are disabled', async () => {
        ...
        const prevFlag = process.env.ALICORN_FEATURE_REMOTE_AGENT_HOOKS
        process.env.ALICORN_FEATURE_REMOTE_AGENT_HOOKS = '0'
        try {
          ...
          expect(env.ALICORN_AGENT_HOOK_TOKEN).toBeUndefined()
          ...
          if (prevFlag === undefined) {
            ...
            process.env.ALICORN_FEATURE_REMOTE_AGENT_HOOKS = prevFlag
          }
        }
      })
      it('forwards ALICORN_PANE_KEY/TAB_ID/WORKTREE_ID over SSH by default', async () => {
        const sshSpawn = vi.fn(async (_opts: { env: Record<string, string> }) => ({
          id: 'ssh-pty'
        }))
        registerSshPtyProvider('ssh-1', {
        ...
        try {
          ...
          await handlers.get('pty:spawn')!(null, {
            ...
            env: {
              ...
              ALICORN_TAB_ID: 'tab-2',
              ALICORN_WORKTREE_ID: 'wt-2'
            },
            connectionId: 'ssh-1',
            tabId: 'tab-2',
            leafId
          ...
          const env = sshSpawn.mock.calls.at(-1)![0].env
          expect(env.ALICORN_PANE_KEY).toBe(paneKey)
          expect(env.ALICORN_TAB_ID).toBe('tab-2')
          expect(env.ALICORN_WORKTREE_ID).toBe('wt-2')
          // Local hook server coords must NOT cross the wire — the relay is the source of truth.
          expect(env.ALICORN_AGENT_HOOK_TOKEN).toBeUndefined()
          expect(env.ALICORN_AGENT_HOOK_PORT).toBeUndefined()
          expect(env.ALICORN_AGENT_HOOK_ENDPOINT).toBeUndefined()
        } finally {
          if (prevFlag === undefined) {
            delete process.env.ALICORN_FEATURE_REMOTE_AGENT_HOOKS
