  ...
  describe('spawn environment', () => {
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('does not mutate the caller-provided args.env on the daemon path', async () => {
        ...
        expect(spawnEnv.ALICORN_AGENT_HOOK_PORT).toBe('5678')
      ...
      it('does NOT inject host-local env on SSH spawns (connectionId set)', async () => {
        ...
        await handlers.get('pty:spawn')!(null, {
          cols: 80,
          rows: 24,
          env: { FOO: 'bar', ALICORN_PANE_KEY: makePaneKey('tab-1', leafId) },
          connectionId: 'ssh-1',
          worktreeId: 'wt-1',
          tabId: 'tab-1',
          leafId
        ...
        await handlers.get('pty:spawn')!(null, {
          cols: 80,
          rows: 24,
          env: { ALICORN_PANE_KEY: 'tab-1:pane:1' },
          connectionId: 'ssh-1',
          worktreeId: 'wt-1',
          tabId: 'tab-1',
          leafId: 'pane:1'
        ...
        expect(legacySpawnOptions?.env.ALICORN_PANE_KEY).toBeUndefined()
