...
export function buildPtyHostEnv(
  ...
  if (opts.agentStatusHooksEnabled) {
    ...
    if (isMimoLaunchCommand(launchCommandHint)) {
      ...
      if (baseEnv.MIMOCODE_HOME) {
        baseEnv.ALICORN_MIMOCODE_HOME = baseEnv.MIMOCODE_HOME
        if (preexistingMimocodeHome) {
          baseEnv.ALICORN_MIMOCODE_SOURCE_HOME = preexistingMimocodeHome
        } else {
          delete baseEnv.ALICORN_MIMOCODE_SOURCE_HOME
        }
      }
    }
  } else {
    restoreOrStripOverlayEnv(baseEnv, {
      ...
      overlay: 'ALICORN_OPENCODE_CONFIG_DIR',
      source: 'ALICORN_OPENCODE_SOURCE_CONFIG_DIR'
    })
    restoreOrStripOverlayEnv(baseEnv, {
      primary: 'MIMOCODE_HOME',
      overlay: 'ALICORN_MIMOCODE_HOME',
      source: 'ALICORN_MIMOCODE_SOURCE_HOME'
    })
  }
  ...
  if (opts.agentStatusHooksEnabled) {
    ...
    if (opts.isWsl === true) {
      ...
      if (guestEndpoint) {
        baseEnv.ALICORN_AGENT_HOOK_ENDPOINT = guestEndpoint
      }
      ...
      if (opencodeOverlayDir) {
        ...
        baseEnv.ALICORN_OPENCODE_CONFIG_DIR = opencodeOverlayDir
        delete baseEnv.ALICORN_OPENCODE_SOURCE_CONFIG_DIR
      } else {
        ...
        delete baseEnv.ALICORN_OPENCODE_CONFIG_DIR
        delete baseEnv.ALICORN_OPENCODE_SOURCE_CONFIG_DIR
      }
    }
  }
  ...
  if (opts.agentStatusHooksEnabled) {
    ...
    restoreOrStripOverlayEnv(baseEnv, {
      ...
      overlay: 'ALICORN_PI_CODING_AGENT_DIR',
      source: 'ALICORN_PI_SOURCE_AGENT_DIR'
    })
    restoreOrStripOverlayEnv(baseEnv, {
      primary: 'PI_CODING_AGENT_DIR',
      overlay: 'ALICORN_OMP_CODING_AGENT_DIR',
      source: 'ALICORN_OMP_SOURCE_AGENT_DIR'
    })
  ...
  if (opts.skipCodexHomeEnv) {
    ...
    delete baseEnv.ALICORN_CODEX_HOME
    delete baseEnv.ALICORN_CODEX_LAUNCH_PREFLIGHT
    ...
    baseEnv.ALICORN_CODEX_HOME = opts.selectedCodexHomePath
    ...
    if (preflightCommand) {
      baseEnv.ALICORN_CODEX_LAUNCH_PREFLIGHT = preflightCommand
    } else {
      delete baseEnv.ALICORN_CODEX_LAUNCH_PREFLIGHT
    }
  ...
  if (opts.isWsl) {
    baseEnv.ALICORN_USER_DATA_PATH = opts.userDataPath
    ...
    baseEnv.ALICORN_CLI_COMMAND = opts.isPackaged ? 'alicorn-ide' : 'orca-dev'
  } else {
    if (!opts.isPackaged) {
      baseEnv.ALICORN_USER_DATA_PATH ??= opts.userDataPath
    }
    delete baseEnv.ALICORN_CLI_COMMAND
  }
  ...
  // Why last, and why here: this is the one place every host PTY's env is finished, and a
  // shell, hook script or CLI shim left over from the previous release reads only the
  // pre-rebrand names. One release of both, then this line goes.
  Object.assign(baseEnv, withLegacyEnvAliases(baseEnv))
