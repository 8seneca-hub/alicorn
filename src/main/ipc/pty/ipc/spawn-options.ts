...
export async function buildPtyIpcSpawnOptions(
  ...
  ctx.spawnEnv = ctx.preAllocatedHandle
    ? { ...ctx.env, ALICORN_TERMINAL_HANDLE: ctx.preAllocatedHandle }
    : ctx.env
  ...
  ctx.combinedEnvToDelete = mergePtyEnvDeletions(
    ...
    // Why: disable old hosts without removing ALICORN_REAL_* while their Windows shim remains on PATH.
    ...
    ctx.stripInheritedOrcaCodexHome ? ['ALICORN_CODEX_HOME'] : []
  )
