...
export async function persistPtyIpcSpawnCommit(ctx: PtyIpcSpawnState): Promise<{
  ...
  if (ctx.deps.store && args.connectionId) {
    // Why: remote PTYs live in the SSH relay grace window after Alicorn detaches; persist IDs immediately so reconnect reattaches instead of spawning a fresh shell.
