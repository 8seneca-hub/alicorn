...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    it('does not use an inherited Pi overlay source for an OMP launch', async () => {
      ...
      expect(env.ALICORN_OMP_SOURCE_AGENT_DIR).toBe('/tmp/default-omp-agent')
      ...
      const env = await spawnAndGetEnv(
        {
          ...
          ALICORN_OMP_CODING_AGENT_DIR: '/tmp/parent-orca-omp-overlay',
          ALICORN_OMP_SOURCE_AGENT_DIR: '/tmp/user-omp-agent'
        },
        undefined,
        undefined,
        undefined,
      ...
      const env = await spawnAndGetEnv(
        {
          ...
          ALICORN_PRIME_AGENT_SOURCE_AGENT_DIR: '/tmp/user-prime-agent'
        },
        undefined,
        undefined,
        () => ({ agentStatusHooksEnabled: false })
    ...
    it('strips stale inherited hook receiver env before injecting this runtime', async () => {
      const env = await spawnAndGetEnv({
        ...
        ALICORN_AGENT_HOOK_VERSION: 'stale-version',
      ...
      expect(env.ALICORN_AGENT_HOOK_PORT).toBe('5678')
      expect(env.ALICORN_AGENT_HOOK_TOKEN).toBe('agent-token')
      expect(env.ALICORN_AGENT_HOOK_ENV).toBeUndefined()
      expect(env.ALICORN_AGENT_HOOK_VERSION).toBeUndefined()
    ...
    it('waits for managed Codex auth before spawning a local PTY', async () => {
      ...
      expect(spawnMock.mock.calls.at(-1)?.[2].env).toMatchObject({
        ...
        ALICORN_CODEX_HOME: TEST_CODEX_HOME
      })
    })
    ...
    it('lets the resolver keep a per-spawn custom CODEX_HOME on the managed lane', async () => {
      ...
      const env = await spawnAndGetEnv(
        ...
        { CODEX_HOME: undefined, ALICORN_CODEX_HOME: undefined },
        resolveHome,
        () => ({ codexSystemDefaultRealHomeEnabled: true }) as never
      )
