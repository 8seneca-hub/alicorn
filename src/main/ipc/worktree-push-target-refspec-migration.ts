...
// Orca-provenance `pr-*` remote's refspec to only the branches Alicorn actually tracked
...
// alone isn't proof of Alicorn provenance the way a metadata entry is.
//
...
/** `pushTarget`-derived branches to keep per remote, gated on at least one entry proving Alicorn created it. */
...
export async function migrateForkRemoteRefspecsWithExec(
  ...
  for (const remoteName of candidateNames) {
    ...
    if (branches.size === 0) {
      ...
      // pre-#17828 Alicorn's), not a `pr`-prefixed remote a user configured by hand. Clears
