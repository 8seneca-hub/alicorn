...
describe('diagnostics IPC handlers', () => {
  beforeEach(() => {
    ...
    delete (globalThis as { ALICORN_BUILD_IDENTITY?: unknown }).ALICORN_BUILD_IDENTITY
    delete (globalThis as { ALICORN_DIAGNOSTICS_TOKEN_URL?: unknown }).ALICORN_DIAGNOSTICS_TOKEN_URL
    process.env.ALICORN_DIAGNOSTICS_TOKEN_URL = 'https://diagnostics.example.com/diagnostics/token'
  ...
  it('pins official builds to the compile-time diagnostics endpoint', async () => {
    ...
    const globalOverrides = globalThis as {
      ALICORN_BUILD_IDENTITY?: 'stable'
      ALICORN_DIAGNOSTICS_TOKEN_URL?: string
    }
    globalOverrides.ALICORN_BUILD_IDENTITY = 'stable'
    globalOverrides.ALICORN_DIAGNOSTICS_TOKEN_URL = 'https://official.example.com/diagnostics/token'
    process.env.ALICORN_DIAGNOSTICS_TOKEN_URL = 'https://attacker.example.com/diagnostics/token'
    collectDiagnosticBundleMock.mockReturnValue(bundle)
    readFileSyncMock.mockReturnValue(bundle.payload)
    const collect = handlers.get('diagnostics:collectBundle')!
    const openPreview = handlers.get('diagnostics:openBundlePreview')!
