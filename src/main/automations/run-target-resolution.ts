...
const CAPTURED_HOST_REFUSALS: Record<AutomationCapturedHostIssue, string> = {
  ...
  [AUTOMATION_ORPHAN_ISSUES.workspaceHostAmbiguous]:
    'The automation workspace spans more than one host, so Alicorn cannot tell which one to run it on.'
}
...
export function resolveAutomationRunTarget(
  ...
  if (
    ...
    return {
      ok: false,
      error:
        'Remote-server automation scheduling is not available from this Alicorn client yet. Run this automation on the remote server or update Alicorn when durable remote scheduling is available.'
    }
  }
