...
export function describeStrandedAutomationRun(run: AutomationRun): string {
  if (run.status === 'dispatching') {
    return 'Alicorn stopped before this run reported that its agent started.'
  }
  return 'Alicorn lost the terminal for this run before it reported completion.'
}
...
function describeObservationError(error: unknown): string {
  ...
  return 'Alicorn stopped watching this run before it reported completion.'
}
