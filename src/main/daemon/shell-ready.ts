...
const ALICORN_USER_DATA_PATH_ENV = 'ALICORN_USER_DATA_PATH'

function getShellReadyWrapperBaseDir(): string {
  const userDataPath = process.env[ALICORN_USER_DATA_PATH_ENV]
  ...
  // ALICORN_USER_DATA_PATH, and daemon startup must not fail before the parent can
...
// ALICORN_USER_DATA_PATH is ever re-pointed mid-process.
...
export function resolvePtyShellPath(env: Record<string, string>): string {
  if (process.platform === 'win32') {
    return env.ALICORN_TERMINAL_WINDOWS_SHELL || 'powershell.exe'
  }
