...
describe('createPtySubprocess', () => {
  ...
  it('ignores the PowerShell implementation setting for cmd.exe on Windows', async () => {
    ...
    try {
      await createPtySubprocess({
        ...
        terminalWindowsPowerShellImplementation: 'pwsh.exe',
        env: { ALICORN_CODEX_LAUNCH_PREFLIGHT: CODEX_LAUNCH_PREFLIGHT }
      })
    } finally {
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({ ALICORN_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE: '"' })
      })
    )
  })
  ...
  it('adds shell and cwd context when node-pty reports File not found on Windows', async () => {
    ...
    const previousVersion = process.env.ALICORN_APP_VERSION
    process.env.ALICORN_APP_VERSION = '1.4.178-test'
    ...
    try {
      ...
      if (previousVersion === undefined) {
        delete process.env.ALICORN_APP_VERSION
      } else {
        process.env.ALICORN_APP_VERSION = previousVersion
      }
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
      }
