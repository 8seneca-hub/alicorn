...
const WSL_COMMAND_NAME = 'alicorn-ide'
...
export async function resolveReadyWslCliState(args: {
  ...
  if (!hostStatus.launcherPath) {
    return {
      status: unsupportedWslCliStatus(
        ...
        hostStatus.detail ?? 'The Windows Alicorn CLI launcher is missing.'
      )
    }
  }
  ...
  if (!interopReady) {
    return {
      status: unsupportedWslCliStatus(
        'launcher_missing',
        'WSL Windows interop is unavailable; Alicorn cannot launch the Windows CLI from WSL.'
      )
    }
  }
