...
describe('CliInstaller', () => {
  ...
  it('creates a windows wrapper and updates the user PATH', async () => {
    ...
    const installPath = join(fixture.root, 'Programs', 'Alicorn', 'bin', 'alicorn.cmd')
    let userPath = 'C:\\Windows\\System32'
    const installer = new CliInstaller({
      platform: 'win32',
      isPackaged: false,
      userDataPath: fixture.userDataPath,
      execPath: 'C:\\Users\\me\\AppData\\Local\\Alicorn\\Alicorn.exe',
    ...
    expect(userPath).toContain(join(fixture.root, 'Programs', 'Alicorn', 'bin'))
    ...
    expect(userPath).not.toContain(join(fixture.root, 'Programs', 'Alicorn', 'bin'))
  })
  ...
  it('skips the Windows PATH write when removing an absent entry', async () => {
    ...
    const installer = new CliInstaller({
      ...
      execPath: 'C:\\Users\\me\\AppData\\Local\\Alicorn\\Alicorn.exe',
      appPath: fixture.appPath,
      commandPathOverride: join(fixture.root, 'Programs', 'Alicorn', 'bin', 'alicorn.cmd'),
      userPathReader: async () => userPathRead('C:\\Windows\\System32'),
      userPathWriter
    })
  ...
  it('matches expandable Windows PATH entries case-insensitively without rewriting them', async () => {
    const fixture = await makeFixture()
    const installPath = join(fixture.root, 'Local App Data', 'Alicorn', 'bin', 'alicorn.cmd')
    const userPathWriter = vi.fn()
    const installer = new CliInstaller({
      platform: 'win32',
      isPackaged: false,
      ...
      userPathReader: async () => userPathRead('%localappdata%\\Alicorn\\bin\\', true),
      userPathWriter
    })
  ...
  it('does not expand environment variables stored in a REG_SZ Windows PATH', async () => {
    ...
    const installer = new CliInstaller({
      ...
      commandPathOverride: installPath,
      windowsEnvironment: { LOCALAPPDATA: join(fixture.root, 'Local App Data') },
      userPathReader: async () => userPathRead('%LOCALAPPDATA%\\Alicorn\\bin'),
      userPathWriter
    })

    await installer.install()

    expect(userPathWriter).toHaveBeenCalledWith(`%LOCALAPPDATA%\\Alicorn\\bin;${pathDirectory}`)
  })
  ...
  it('resolves custom-install packaged Windows command path from resourcesPath', async () => {
    ...
    const resourcesPath = join(fixture.root, 'D Custom Alicorn', 'resources')
    await mkdir(join(resourcesPath, 'bin'), { recursive: true })
    await writeFile(join(resourcesPath, 'bin', 'alicorn.exe'), 'native launcher', 'utf8')
    ...
    const installer = new CliInstaller({
      ...
      localAppDataPath,
      userDataPath: fixture.userDataPath,
      execPath: join(fixture.root, 'D Custom Alicorn', 'Alicorn.exe'),
      appPath: fixture.appPath,
    ...
    expect(status.commandPath).toBe(join(resourcesPath, 'bin', 'alicorn.exe'))
  })
  ...
  it('keeps a bundled Windows launcher installed when the user PATH read is unknown', async () => {
    ...
    const bundledLauncher = join(resourcesPath, 'bin', 'alicorn.exe')
    ...
    const installer = new CliInstaller({
      ...
      resourcesPath,
      userDataPath: fixture.userDataPath,
      execPath: join(fixture.root, 'Alicorn.exe'),
      appPath: fixture.appPath,
      userPathReader: async () => ({
        state: 'unknown',
        detail: 'Alicorn could not read the Windows user PATH registry value.'
