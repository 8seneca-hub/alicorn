...
export class CliInstaller extends CliPathRegistration {
  ...
  async getStatus(): Promise<CliInstallStatus> {
    ...
    if (!launcherPath) {
      const detail = this.hasUnverifiedAppImageRuntime
        ? 'Alicorn could not verify the inherited AppImage runtime identity, so CLI registration is unavailable.'
        : this.isLinuxAppImage() && this.appImagePath
          ...
          : this.isPackaged
            ? 'The bundled CLI launcher is missing from this Alicorn build.'
    ...
    const baseStatus =
      ...
      !isAppImageInstalledLauncherCurrent(extractionOptions)
        ? {
            ...
            detail: `${spec.commandPath} does not point to the current Alicorn AppImage payload.`
          }
        : inspectedStatus
  ...
  private async installUnlocked(): Promise<CliInstallStatus> {
    ...
    if (initialStatus.state === 'conflict') {
      throw new Error(
        `Refusing to replace non-Alicorn command at ${initialStatus.commandPath}. Remove it and register again if it is no longer needed.`
      )
    }
  ...
  private async removeUnlocked(): Promise<CliInstallStatus> {
    ...
    if (status.state === 'stale' && status.installMethod !== 'symlink') {
      throw new Error(`Refusing to remove a command not owned by Alicorn at ${status.commandPath}.`)
    }
