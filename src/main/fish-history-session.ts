 ...
 * Alicorn's history tree. That is why deletion needs its own path here instead of
 ...
 *  CLIENT's worktree ids shares the one fish data dir with any desktop Alicorn on
...
/** Drop a `fish_history` this process inherited from an outer Alicorn.
 *  fish EXPORTS the variable, so an Alicorn launched from a fish pane hands the
 ...
 *  Resolve this from the PTY's SPAWN env: Alicorn can be launched with a different
 ...
 *  the two disagree whenever Alicorn's environment differs from the PTY's. */
 ...
 * scan of the data dir can find those. Matching on Alicorn's own `orca_<hex>_`
 ...
 * outside Alicorn's own directory.
 */
