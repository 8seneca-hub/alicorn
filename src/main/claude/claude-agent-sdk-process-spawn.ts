...
export type ClaudeCodeProcessSpawn = {
  ...
  /** The retained child, so Alicorn keeps its own tree-kill and exit-proof ladder. Null until the SDK spawns. */
...
/**
 * Alicorn supplies the Claude Code child rather than letting the SDK spawn it.
 *
 * Two independent reasons: the SDK's `SpawnedProcess` has no pid, and Alicorn's
...
export function createClaudeCodeProcessSpawn(
  ...
  return {
    spawn: (options) => {
      ...
      // Alicorn's ladder, and close() may never report an exit it did not observe.
