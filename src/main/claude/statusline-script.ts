...
// it gives Alicorn live usage without spending the OAuth usage endpoint's tight budget.
...
export function getManagedStatusLineScript(target: 'local' | 'posix' = 'local'): string {
  if (target === 'local' && process.platform === 'win32') {
    return [
      ...
      // Why exit, not the drain label: a worker is outside an Alicorn pane, so reading stdin to
      ...
      // Why: call the endpoint file to refresh port/token — a PTY that survived an Alicorn restart carries stale env; falls through to PTY env if missing.
