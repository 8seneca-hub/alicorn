...
/**
 * The managed status hook script for Claude-compatible agents — the `/hook/<source>` post that
 * feeds the sidebar. Split from `hook-service.ts`, which owns install/remove policy, at the
 * line cap; `statusline-script.ts` and the per-agent `hook-script.ts` files are the precedent.
 */
