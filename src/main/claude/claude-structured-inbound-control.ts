...
/**
 * The blocking control requests Alicorn answers, each mapped to the SDK consumer callback that
 ...
 * blocking control Alicorn must answer means adding its callback here, and the catalog test
...
function denySafeResult(toolUseId: string | undefined): PermissionResult {
  return {
    behavior: 'deny',
    message: 'Alicorn could not decode this permission request.',
 ...
 * by `answerClaudePrompt`. `onUserDialog` is deny-safe; the CLI only emits dialog kinds Alicorn
