...
export abstract class UpdaterInstallSupport extends UpdaterCheckState {
  ...
  protected deferHeadlessServeInstall(phase: 'download' | 'install', version: string): boolean {
    ...
    this.sendErrorStatus(
      'This orca serve process was not started by an update-capable supervisor. Keep it running and update Alicorn through its service manager.',
      true
    )
    return true
  }
  ...
  protected getPreCommitInstallFailureMessage(): string {
    return process.platform === 'darwin'
      ? 'Could not restart to install the update. Quit and reopen Alicorn, then try again.'
      : 'Could not start the update installer. Alicorn remains open.'
  }
