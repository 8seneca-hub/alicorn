...
describe('Windows managed hook stdin structure', () => {
  it('exits immediately when Alicorn env is missing and keeps drain for other failures', async () => {
    ...
    try {
      ...
      // must sit below the env guards — otherwise a Devin session outside an Alicorn pane still
  ...
  it.skipIf(process.platform !== 'win32')(
    'exits 0 for every local script and missing-script launcher, dropping stdin only without Alicorn env',
    async () => {
      ...
      try {
        ...
        for (const fileName of mainScripts) {
          ...
          // Alicorn env is missing, so the writer may break — EPIPE, or ECONNRESET when Windows
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      try {
        ...
        const environments = [
          { name: 'no Alicorn env', env: hookEnvironment({ USERPROFILE: home }) },
          {
            name: 'Alicorn env with dead listener',
            env: hookEnvironment({
              USERPROFILE: home,
...
describe.skipIf(process.platform === 'win32')('managed hook stdin lifecycle', () => {
  ...
  it('accepts a large payload without Alicorn environment or a broken writer', async () => {
    const scripts = await generatePosixScripts()
    for (const [agent, script] of scripts) {
