...
// user-global (~/.claude/settings.json, ~/.cursor/hooks.json). A second Alicorn profile with the off
...
// Alicorn's script, but the presence gate below then skips install() forever, freezing the
// script at whatever Alicorn generated last. Existing scripts are Orca-owned, so bring them
