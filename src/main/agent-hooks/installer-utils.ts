...
// Why: prod/dev/parallel Alicorn instances must write the same managed entry, not race between per-userData script paths.
...
export function buildWindowsAgentHookPostCommand(
  ...
  return [
    `"%SystemRoot%\\System32\\curl.exe" -sS -X POST "http://127.0.0.1:%ALICORN_AGENT_HOOK_PORT%/hook/${source}" ^`,
...
export function buildWindowsAgentHookCurlPostCommand(source: AgentHookSource): string {
  return [
    ...
    `"http://127.0.0.1:%ALICORN_AGENT_HOOK_PORT%/hook/${source}"`,
    '--connect-timeout 0.5 --max-time 1.5',
    '-H "Content-Type: application/x-www-form-urlencoded"',
    '-H "X-Orca-Agent-Hook-Token: %ALICORN_AGENT_HOOK_TOKEN%"',
    '--data-urlencode "paneKey=%ALICORN_PANE_KEY%"',
