...
export function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  if (target === 'local' && process.platform === 'win32') {
    return [
      ...
      // lines so surviving PTYs can refresh to the current Alicorn server.
      'if ($env:ALICORN_AGENT_HOOK_ENDPOINT -and (Test-Path -LiteralPath $env:ALICORN_AGENT_HOOK_ENDPOINT)) {',
      '  try {',
      '    Get-Content -LiteralPath $env:ALICORN_AGENT_HOOK_ENDPOINT | ForEach-Object {',
      ...
      '  } catch {}',
      '}',
      // Why (#11549 class): missing Alicorn context means a user-wide hook fired outside an
      // Alicorn pane. ReadToEnd blocks forever if that caller abandons the pipe, so the guard
      // must run before the hook owns stdin; the payload would be discarded anyway.
      'if (-not $env:ALICORN_AGENT_HOOK_PORT -or -not $env:ALICORN_AGENT_HOOK_TOKEN -or -not $env:ALICORN_PANE_KEY) { exit 0 }',
      '$inputData = [Console]::In.ReadToEnd()',
      'if ([string]::IsNullOrWhiteSpace($inputData)) { exit 0 }',
      'try {',
      '  $payload = $inputData | ConvertFrom-Json',
      '  $body = @{',
      '    paneKey = $env:ALICORN_PANE_KEY',
      '    launchToken = $env:ALICORN_AGENT_LAUNCH_TOKEN',
      '    tabId = $env:ALICORN_TAB_ID',
      '    worktreeId = $env:ALICORN_WORKTREE_ID',
      '    hookEventName = $env:ALICORN_COPILOT_HOOK_EVENT',
      '    env = $env:ALICORN_AGENT_HOOK_ENV',
      '    version = $env:ALICORN_AGENT_HOOK_VERSION',
      '    payload = $payload',
      '  } | ConvertTo-Json -Depth 100',
      "  Invoke-WebRequest -UseBasicParsing -Method Post -Uri ('http://127.0.0.1:' + $env:ALICORN_AGENT_HOOK_PORT + '/hook/copilot') -Headers @{ 'Content-Type'='application/json'; 'X-Orca-Agent-Hook-Token'=$env:ALICORN_AGENT_HOOK_TOKEN } -Body $body -TimeoutSec 2 | Out-Null",
      '} catch {}',
      'exit 0',
      ''
    ].join('\r\n')
  ...
  return [
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${ALICORN_AGENT_HOOK_PORT}/hook/copilot" \\',
    ...
    '  --data-urlencode "hookEventName=${ALICORN_COPILOT_HOOK_EVENT}" \\',
