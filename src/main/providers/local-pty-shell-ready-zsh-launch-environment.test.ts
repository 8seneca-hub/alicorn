...
describePosix('live zsh subprocess tests', () => {
  describeIfZsh('terminal emulator edge cases', () => {
    ...
    it('discovers ZDOTDIR when launched inside tmux', async () => {
      ...
      delete cleanEnv.ALICORN_ORIG_ZDOTDIR
    ...
    it('re-discovers ZDOTDIR despite stale ALICORN_ORIG_ZDOTDIR from previous session', async () => {
      ...
      const previousOrcaZdotdir = process.env.ALICORN_ORIG_ZDOTDIR
      process.env.ALICORN_ORIG_ZDOTDIR = '/opt/orca-old/shell-ready/zsh' // stale wrapper path
      ...
      try {
        ...
        const cleanEnv: Record<string, string | undefined> = {
          ...process.env,
          HOME: testHome,
          ALICORN_ORIG_ZDOTDIR: '/opt/orca-old/shell-ready/zsh'
        }
        delete cleanEnv.ZDOTDIR
        cleanEnv.ZDOTDIR = config.env.ZDOTDIR
        ...
        if (previousOrcaZdotdir === undefined) {
          ...
          process.env.ALICORN_ORIG_ZDOTDIR = previousOrcaZdotdir
        }
      }
    })

    it('prioritizes fresh discovery over inherited ALICORN_ORIG_ZDOTDIR', async () => {
      ...
      process.env.ALICORN_ORIG_ZDOTDIR = oldZdotdir
      ...
      try {
        ...
        const cleanEnv: Record<string, string | undefined> = {
          ...process.env,
          HOME: testHome,
          ALICORN_ORIG_ZDOTDIR: oldZdotdir
        }
        delete cleanEnv.ZDOTDIR
        cleanEnv.ZDOTDIR = config.env.ZDOTDIR
    ...
    it('sources launch-time ZDOTDIR .zshenv when it is explicitly inherited', async () => {
      ...
      try {
        const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
        const config = getShellReadyLaunchConfig('/bin/zsh')
        // One channel now does what ALICORN_ORIG_ZDOTDIR and ALICORN_ZSHENV_SOURCE_DIR
        // split between them: the wrapper hands this back as ZDOTDIR, and zsh
        // reads .zshenv through it.
        expect(config.env.ALICORN_ORIG_ZDOTDIR).toBe(inheritedZdotdir)
