...
describe('shell-ready wrapper root resolution', () => {
  // Why: daemon-entry fork is plain Node (no electron), so the wrapper root resolves from ALICORN_USER_DATA_PATH, not app.getPath.
  it('resolves the wrapper root from ALICORN_USER_DATA_PATH', async () => {
...
describePosix('local PTY shell-ready launch config', () => {
  ...
  it('falls back to HOME when inherited ALICORN_ORIG_ZDOTDIR points at a wrapper dir', async () => {
    const previousZdotdir = process.env.ZDOTDIR
    const previousOrigZdotdir = process.env.ALICORN_ORIG_ZDOTDIR
    const previousHome = process.env.HOME
    delete process.env.ZDOTDIR
