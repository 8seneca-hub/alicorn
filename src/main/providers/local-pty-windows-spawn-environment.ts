...
import { ALICORN_HERMES_STARTUP_QUERY_ENV } from '../../shared/hermes-startup-query'
...
import {
  ALICORN_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE_ENV,
  ...
  if (pathWin32.basename(plan.shellPath).toLowerCase() === 'wsl.exe') {
    if (codexHomeWslInfo) {
      if (plan.launchWslDistro && plan.launchWslDistro !== codexHomeWslInfo.distro) {
        ...
        env.ALICORN_CODEX_HOME = codexHomeWslInfo.linuxPath
        // Why: wsl.exe only imports non-default env vars named in WSLENV.
        addWslEnvKeys(env, ['CODEX_HOME', 'ALICORN_CODEX_HOME'])
    ...
    if (env[ALICORN_HERMES_STARTUP_QUERY_ENV] !== undefined) {
      // Why: wsl.exe drops custom Windows env vars; the startup wrapper needs this imported inside WSL.
      addWslEnvKeys(env, [ALICORN_HERMES_STARTUP_QUERY_ENV])
    }
  } else if (codexHomeWslInfo || isWslCodexHomeForHost(env.CODEX_HOME)) {
    // Why: WSL Codex homes are Linux paths Windows can't use; also drop ALICORN_CODEX_HOME (shell-ready restores CODEX_HOME from it).
    delete env.CODEX_HOME
    delete env.ALICORN_CODEX_HOME
  }

  const shellBasename = pathWin32.basename(plan.shellPath).toLowerCase()
  const codexLaunchPreflightCommand = env.ALICORN_CODEX_LAUNCH_PREFLIGHT
  if (
    ...
    if (shellBasename === 'cmd.exe') {
      ...
      env[ALICORN_CODEX_LAUNCH_PREFLIGHT_CMD_QUOTE_ENV] = '"'
    }
    const resolved = resolveWindowsShellLaunchArgs(
      plan.shellPath,
      plan.cwd,
