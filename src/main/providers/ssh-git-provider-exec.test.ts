...
describe('SshGitProvider', () => {
  ...
  it('reports an actionable reconnect message when the relay does not support cloning', async () => {
    ...
    await expect(
      ...
      'SSH clone support is unavailable on this relay. Reconnect the SSH target to update Alicorn on the host, then try again.'
    )
  })
  ...
  it('execNonInteractive forwards environment variables to the relay', async () => {
    ...
    await provider.execNonInteractive(
      '/bin/bash',
      ['-lc', 'echo "$ALICORN_WORKTREE_PATH"'],
      '/home/user/repo',
      120_000,
      undefined,
      {
        ALICORN_ROOT_PATH: '/home/user/repo',
        ALICORN_WORKTREE_PATH: '/home/user/repo-feature'
      }
    )
    ...
    expect(mux.request).toHaveBeenCalledWith(
      ...
      {
        binary: '/bin/bash',
        args: ['-lc', 'echo "$ALICORN_WORKTREE_PATH"'],
        cwd: '/home/user/repo',
        stdin: null,
        timeoutMs: 120_000,
        env: {
