...
export function buildLocalPtySpawnEnvironment(args: {
  ...
  const spawnEnv: Record<string, string> = {
    ...
    // Why: supports-hyperlinks rejects TERM_PROGRAM=Alicorn, so tools drop OSC 8 links; force it since xterm.js parses them.
    FORCE_HYPERLINK: '1'
  } as Record<string, string>
  // Why: Alicorn can be launched from an Alicorn terminal; pane identity belongs to the child PTY, not the parent shell.
