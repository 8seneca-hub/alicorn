...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('ALICORN_OPENCODE_CONFIG_DIR')
    expect(script).toContain('ALICORN_MIMOCODE_HOME')
    expect(script).not.toContain('ALICORN_PI_CODING_AGENT_DIR')
    expect(script).not.toContain('ALICORN_OMP_CODING_AGENT_DIR')
    expect(script).toContain('ALICORN_OMP_STATUS_EXTENSION')
    expect(script).toContain('function Global:omp')
    expect(script).toContain('--extension $env:ALICORN_OMP_STATUS_EXTENSION')
    expect(script).toContain('ALICORN_CODEX_HOME')
    expect(script).toContain('ALICORN_CODEX_LAUNCH_PREFLIGHT')
    ...
    const codexHomeRestore = script.indexOf('if ($env:ALICORN_CODEX_HOME)')
