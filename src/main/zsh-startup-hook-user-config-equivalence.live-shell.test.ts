...
const REPORTED = ['ZDOTDIR', 'ALICORN_TEST_MARK', 'ALICORN_TEST_FROM_ZSHRC', 'PATH'] as const
...
const CASES: ConfigCase[] = [
  {
    ...
    setup: (home) => {
      writeFileSync(join(home, '.zshenv'), 'export ALICORN_TEST_MARK=plain\n')
      return home
    }
  },
  {
  ...
  },
  {
    ...
    setup: (home) => {
      ...
      writeFileSync(join(home, '.zshenv'), `export ALICORN_TEST_MARK=sourced\nsource "${common}"\n`)
      return dir
    }
  },
  {
    ...
    setup: (home) => {
      ...
      writeFileSync(
        join(home, '.zshenv'),
        `export ALICORN_TEST_MARK=spaces\nexport ZDOTDIR="${dir}"\n`
      )
      return dir
    }
  },
  {
    ...
    setup: (home) => {
      ...
      writeFileSync(
        join(home, '.zshenv'),
        `export ALICORN_TEST_MARK=twice\nexport ZDOTDIR="${first}"\nexport ZDOTDIR="${dir}"\n`
      )
      return dir
    }
  },
  {
    ...
    setup: (home) => {
      ...
      writeFileSync(
        join(home, '.zshenv'),
        `export ALICORN_TEST_MARK=trailing\nexport ZDOTDIR="${dir}/"\n`
      )
      return dir
    }
  },
  {
    ...
    setup: (home) => {
      writeFileSync(
        join(home, '.zshenv'),
        `export ALICORN_TEST_MARK=missing\nexport ZDOTDIR="${join(home, 'nope')}"\n`
      )
      return home
    }
  },
  {
    ...
    setup: (home) => {
      writeFileSync(join(home, '.zshenv'), 'export ALICORN_TEST_MARK=empty\nexport ZDOTDIR=""\n')
      return home
    }
  },
  {
  ...
  },
  {
    ...
    setup: (home) => {
      writeFileSync(join(home, '.zshenv'), 'export ALICORN_TEST_MARK=broken\nif [ ; then\n')
      return home
    }
  },
  {
  ...
  },
  {
    ...
    setup: (home) => {
      writeFileSync(
        join(home, '.zshenv'),
        'setopt extendedglob nullglob\nexport ALICORN_TEST_MARK=globs\n'
      )
      return home
    }
  },
  {
    ...
    setup: (home) => {
      writeFileSync(join(home, '.zshenv'), 'export ALICORN_TEST_MARK=nohome\nunset HOME\n')
      return home
    }
  },
  {
    ...
    setup: (home) => {
      writeFileSync(
        join(home, '.zshenv'),
        'export ALICORN_TEST_MARK=slashes\nexport ZDOTDIR="///"\n'
      )
      return home
    }
  },
  {
    ...
    setup: (home) => {
      writeFileSync(
        join(home, '.zshenv'),
        'export ALICORN_TEST_MARK=blank\nexport ZDOTDIR="$(printf \'\\t\\n\')"\n'
      )
      return home
    }
  },
  ...
  },
  {
    ...
    setup: (home) => {
      writeFileSync(
        join(home, '.zshenv'),
        'export ALICORN_TEST_MARK=conditional\nexport ZDOTDIR="$HOME/x"\nunset ZDOTDIR\n'
      )
      return home
    }
  },
  {
    ...
    setup: (home) => {
      ...
      writeFileSync(
        join(home, '.zshenv'),
        'typeset -U path\npath=(/usr/bin /bin /usr/bin)\nexport ALICORN_TEST_MARK=uniqpath\n'
      )
      return home
    }
  },
  {
    ...
    setup: (home) => {
      ...
      writeFileSync(
        join(home, '.zshenv'),
        `fpath=(${JSON.stringify(fns)} $fpath)\norca_test_fn() { : }\nexport ALICORN_TEST_MARK=fnscope\n`
      )
      return home
    }
  },
  {
    ...
    setup: (home) => {
      writeFileSync(join(home, '.zshenv'), 'export ALICORN_TEST_MARK=exiting\nexit 0\n')
      return home
    }
  }
]
...
function wrappedEnv(home: string): Record<string, string> {
  const features = selectShellStartupFeatures({
    shellPath: ZSH_PATH,
    env: { HOME: home, ALICORN_HISTFILE: join(home, 'scoped_history') },
    ...
    it.each(CASES.map((testCase) => [testCase.name, testCase] as const))(
      ...
      async (_name, testCase) => {
        ...
        try {
          ...
          writeFileSync(join(zshrcDir, '.zshrc'), 'export ALICORN_TEST_FROM_ZSHRC=1\n')
...
describe.skipIf(process.platform === 'win32')('the fixes the old wrapper was built for', () => {
   ...
   * be able to show the wrapper ran. ALICORN_SHELL_FEATURES coming back consumed is
  ...
  async function runFromRelocatedRoot(home: string, movedRoot: string) {
    ...
    try {
      return await runZshPty({
        ...
        report: ['ALICORN_TEST_FROM_ZSHRC', 'ALICORN_SHELL_FEATURES', 'HISTFILE']
      })
    } finally {
  ...
  itWithZsh(
    ...
    async () => {
      ...
      try {
        ...
        expect(values.ALICORN_TEST_FROM_ZSHRC).toBe('1')
  ...
  itWithZsh('gives the user’s startup files their own ZDOTDIR while they run (#4667)', async () => {
    ...
    writeFileSync(join(xdg, '.zshrc'), 'export ALICORN_TEST_IN_ZSHRC="$ZDOTDIR"\n')
    writeFileSync(join(xdg, '.zprofile'), 'export ALICORN_TEST_IN_ZPROFILE="$ZDOTDIR"\n')
    try {
      const report = ['ALICORN_TEST_IN_ZSHRC', 'ALICORN_TEST_IN_ZPROFILE']
      ...
      expect(wrapped.values.ALICORN_TEST_IN_ZSHRC).toBe(xdg)
      expect(wrapped.values.ALICORN_TEST_IN_ZPROFILE).toBe(xdg)
  ...
  itWithZsh('refuses an inherited ZDOTDIR that is an Alicorn wrapper dir (#15258)', async () => {
    ...
    writeFileSync(join(foreign, '.zshrc'), 'export ALICORN_TEST_FROM_WRAPPER_DIR=1\n')
    writeFileSync(join(foreign, ZSH_WRAPPER_DIR_MARKER_FILE), '')
    try {
      const { values } = await runZshPty({
        env: { ...wrappedEnv(home), ALICORN_ORIG_ZDOTDIR: foreign },
        report: ['ZDOTDIR', 'ALICORN_TEST_FROM_ZSHRC', 'ALICORN_TEST_FROM_WRAPPER_DIR']
      })

      // Rejected, so zsh falls back to $HOME and the user's own config loads.
      expect(values.ZDOTDIR).toBe('UNSET')
      expect(values.ALICORN_TEST_FROM_ZSHRC).toBe('1')
      expect(values.ALICORN_TEST_FROM_WRAPPER_DIR).toBe('UNSET')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  })
  ...
  itWithZsh('leaves a nested Alicorn nothing of its own to inherit (#11044, #11146)', async () => {
    ...
    try {
      const { values } = await runZshPty({
        env: wrappedEnv(home),
        commands: [
          'ALICORN_CHILD_ENV="$(env | grep -cE \'^(ALICORN_SHELL_FEATURES|ALICORN_HISTFILE)=\' || true)"',
          'ALICORN_CHILD_ZDOTDIR="$(env | sed -n \'s/^ZDOTDIR=//p\')"'
        ],
      ...
      expect(values.ALICORN_CHILD_ZDOTDIR).toBe('UNSET')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  })
  ...
  itWithZsh('survives a user .zshenv that returns early (#1947)', async () => {
    const home = makeZshHome({
      '.zshenv': 'export ALICORN_TEST_MARK=early\nreturn 0\nexport ALICORN_TEST_MARK=late\n',
      '.zshrc': 'export ALICORN_TEST_FROM_ZSHRC=1\n'
    })
    try {
      const report = ['ALICORN_TEST_MARK', 'ALICORN_TEST_FROM_ZSHRC', 'ZDOTDIR']
      const wrapped = await runZshPty({ env: wrappedEnv(home), report })
      const unwrapped = await runZshPty({ env: { PATH: '/usr/bin:/bin', HOME: home }, report })

      expect(wrapped.values).toEqual(unwrapped.values)
      expect(wrapped.values.ALICORN_TEST_FROM_ZSHRC).toBe('1')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  })
