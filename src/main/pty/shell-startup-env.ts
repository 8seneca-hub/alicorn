...
// Why: Alicorn launches bash as a login shell (see local-pty-shell-ready.ts
...
// .bashrc. Scanning .bashrc would mirror values the live Alicorn bash never sees.
...
function shellStartupFiles(
  ...
  if (!shell) {
    // Why: Alicorn's POSIX default shell is /bin/zsh when $SHELL is unset.
  ...
  // Alicorn's zsh/bash/fish shell-ready startup files, so scanning those files
 ...
 *
 * Why: GUI-launched Alicorn does not inherit interactive shell exports, but the
