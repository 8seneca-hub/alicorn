...
describe('ssh host key store', () => {
  ...
  it('scopes trust to the endpoint, so a second target naming the same host is already trusted', async () => {
    ...
    // A different Alicorn target, same machine — no target id is recorded anywhere.
 ...
 *
 * `version` was written and never read, so a store from a future Alicorn would have every record
