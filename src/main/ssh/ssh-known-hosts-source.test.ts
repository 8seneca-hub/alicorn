...
describe('resolveKnownHostsLookupHost', () => {
  ...
  // INVERTED from 'uses the resolved hostname, never the Alicorn label'. That test encoded an
  // assumption verified false against OpenSSH 10.2p1: `ssh -G` echoes its own argument back as
  // `hostname` when no Host block matches, so for a manual target `resolved.hostname` IS the Alicorn
