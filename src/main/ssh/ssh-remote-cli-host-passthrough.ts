...
// switch (the cause of "Unsupported SSH Alicorn CLI command", #7716), the host
...
// Why: only Alicorn terminal-context vars may cross from the remote shell into
...
export async function runHostOrcaCliPassthrough(
  ...
  try {
    ...
    // reports "Alicorn is not running" against a healthy app.
  ...
  if (!entryExists(cliEntryPath)) {
    throw new HostCliUnavailableError(`Alicorn CLI entry not found at ${cliEntryPath}`)
  }
  ...
  return await new Promise<RemoteOrcaCliResult>((resolve, reject) => {
    ...
    const killTimer = setTimeout(() => {
      ...
      resolve({
        ...
        stderr: `${stderr.toString()}Alicorn CLI bridge timed out after ${killTimeoutMs}ms on the host.\n`,
        exitCode: 1
      })
    }, killTimeoutMs)
    killTimer.unref?.()
    ...
    child.on('error', (err) => {
      ...
      reject(
        new HostCliUnavailableError(`Failed to launch the Alicorn CLI on the host: ${err.message}`)
      )
    })
