...
process.stdin.on('data', () => {
  process.stderr.write(process.env.ALICORN_STUB_CODEX_STDERR ?? '')
  process.exit(Number(process.env.ALICORN_STUB_CODEX_EXIT_CODE ?? '1'))
})
`
...
function runStub(stderr: string, exitCode: number): Promise<{ error: string | null }> {
  process.env.ALICORN_STUB_CODEX_STDERR = stderr
  process.env.ALICORN_STUB_CODEX_EXIT_CODE = String(exitCode)
...
describe('Codex RPC exit diagnostics', () => {
  ...
  afterEach(() => {
    delete process.env.ALICORN_STUB_CODEX_STDERR
    delete process.env.ALICORN_STUB_CODEX_EXIT_CODE
