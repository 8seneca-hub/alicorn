...
if (process.env.CODEX_HOME !== process.env.ALICORN_EXPECTED_CODEX_HOME) {
...
describe('Codex rate-limit process contract', () => {
  beforeEach(() => {
    ...
    previousExpectedHome = process.env.ALICORN_EXPECTED_CODEX_HOME
    process.env.ALICORN_EXPECTED_CODEX_HOME = join(tempRoot, 'managed-codex-home')
  ...
  afterEach(() => {
    if (previousExpectedHome === undefined) {
      delete process.env.ALICORN_EXPECTED_CODEX_HOME
    } else {
      process.env.ALICORN_EXPECTED_CODEX_HOME = previousExpectedHome
    }
  ...
  it('starts a read-only non-interactive app-server with the managed home', async () => {
    await expect(
      fetchCodexRateLimits({
        codexHomePath: process.env.ALICORN_EXPECTED_CODEX_HOME,
