      ...
      try {
        ...
        toast.warning(
          translate(
            ...
            'macOS permissions may not reach Alicorn terminals'
          ),
          {
            id: SEVERED_TCC_NOTICE_ID,
            description: translate(
              'auto.hooks.useMacTccAttributionSeveredNotice.description',
              'Running Alicorn terminals are hosted by a daemon started by a previous Alicorn installation. macOS may not apply Alicorn’s Accessibility, Automation, or protected-file permissions to them. Restart the daemon from Manage Sessions to restore access. This will close all running Alicorn terminals.'
            ),
            duration: Infinity,
            action: {
              label: translate(
