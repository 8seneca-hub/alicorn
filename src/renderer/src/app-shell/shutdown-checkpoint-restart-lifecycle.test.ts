...
import {
  ALICORN_RENDERER_SHUTDOWN_CHECKPOINT_ABORTED_EVENT,
  ALICORN_RENDERER_UNLOAD_PREVENTED_EVENT
...
function createLifecycleHarness(
  ...
  window.addEventListener(ALICORN_RENDERER_UNLOAD_PREVENTED_EVENT, guard.abandonAttempt)
  return {
    stageBeforeUnloadSync,
    prepare: () =>
    ...
    cleanup: () => {
      ...
      window.removeEventListener(ALICORN_RENDERER_UNLOAD_PREVENTED_EVENT, guard.abandonAttempt)
    }
  }
}
...
describe('shutdown checkpoint restart lifecycle', () => {
  ...
  it.each([
    {
      ...
      startedEventName: ALICORN_APP_RESTART_STARTED_EVENT,
      abortedEventName: ALICORN_APP_RESTART_ABORTED_EVENT
    },
    {
      lifecycle: 'updater install',
      startedEventName: ALICORN_UPDATER_QUIT_AND_INSTALL_STARTED_EVENT,
      abortedEventName: ALICORN_UPDATER_QUIT_AND_INSTALL_ABORTED_EVENT
    }
  ])(
  ...
  it('abandons retry state when a later restart attempt is independently canceled', async () => {
    ...
    await expect(harness.prepare()).rejects.toThrow('deterministic full-stage failure')
    window.dispatchEvent(new Event(ALICORN_APP_RESTART_STARTED_EVENT))
    window.dispatchEvent(new Event(ALICORN_APP_RESTART_ABORTED_EVENT))
    await expect(harness.prepare()).rejects.toThrow('deterministic full-stage failure')
