...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `Alicorn pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior Alicorn session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest Alicorn status hints:', ...statusHints] : []),
    '',
