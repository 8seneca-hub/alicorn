...
import { ALICORN_RENDERER_UNLOAD_PREVENTED_EVENT } from '../../../shared/renderer-shutdown-events'
import { ALICORN_APP_RESTART_ABORTED_EVENT } from '../../../shared/updater-renderer-events'
import {
  ALICORN_EDITOR_PREPARE_HOT_EXIT_EVENT,
...
describe('loadLazyWithRetry recovery reload vs the dirty-editor-tab unload veto', () => {
  ...
  function installDirtyEditorTab(options: { hotExitBackupFails?: boolean } = {}): Harness {
    ...
    window.addEventListener(ALICORN_EDITOR_PREPARE_HOT_EXIT_EVENT, hotExitBackup)
    vi.spyOn(window.location, 'reload').mockImplementation(() => {
      ...
      if (!accepted) {
        window.dispatchEvent(new Event(ALICORN_RENDERER_UNLOAD_PREVENTED_EVENT))
      }
    })

    cleanupHarness = () => {
      window.removeEventListener('beforeunload', dirtyTabGuard)
      window.removeEventListener(ALICORN_EDITOR_PREPARE_HOT_EXIT_EVENT, hotExitBackup)
      cleanupBypass()
    }
    return harness
  }
  ...
  it('refuses to reload when unsaved buffers cannot be backed up', async () => {
    ...
    window.addEventListener(ALICORN_APP_RESTART_ABORTED_EVENT, restartAborted)
    const error = chunkParseError()
    ...
    window.removeEventListener(ALICORN_APP_RESTART_ABORTED_EVENT, restartAborted)
  })
