...
export function getSkillFreshnessDisplayStatus(
  ...
  for (const installation of inventory?.installations ?? []) {
    ...
    // drift Alicorn has no way to fix. Skipped before `hasPlacement` so a repo-only skill
    // reports presence, not a freshness claim about a copy Alicorn does not manage.
  ...
  // fail-closed — but only for skills Alicorn actually found somewhere. Flagging a skill
