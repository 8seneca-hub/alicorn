...
describe('launchAgentInNewTab', () => {
  ...
  it('surfaces a toast when host agent launch fails in paired web clients', async () => {
    ...
    mockCreateWebRuntimeSessionTerminal.mockResolvedValue({
      status: 'failed',
      message: 'Upgrade the remote Alicorn host before starting or resuming agent sessions.'
    })
    store.settings = {
      agentCmdOverrides: {},
      agentDefaultArgs: {},
    ...
    expect(mockToastError).toHaveBeenCalledWith(
      'Upgrade the remote Alicorn host before starting or resuming agent sessions.'
    )
  ...
  it('seeds working after Command Code submit-after-ready prompt delivery', async () => {
    ...
    expect(mockSetAgentStatus).toHaveBeenCalledWith(
      `tab-1:${LEAF_ID}`,
      {
        ...
        // Why: seeded from Alicorn's own prompt delivery, not a provider hook (STA-4293).
