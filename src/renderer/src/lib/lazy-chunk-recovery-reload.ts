...
function waitForRefusedNavigation(win: Window): RefusedNavigationWait {
  ...
  const cancel = (): void => {
    ...
    win.removeEventListener(ALICORN_RENDERER_UNLOAD_PREVENTED_EVENT, onUnloadPrevented)
  }
  const outcome = new Promise<'unload-vetoed' | 'never-landed'>((resolve) => {
    ...
    win.addEventListener(ALICORN_RENDERER_UNLOAD_PREVENTED_EVENT, onUnloadPrevented)
...
export async function requestLazyChunkRecoveryReload(
  ...
  try {
    ...
    win.dispatchEvent(new Event(ALICORN_APP_RESTART_ABORTED_EVENT))
  }
}
