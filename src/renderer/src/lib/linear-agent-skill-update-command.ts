...
export type LinearAgentSkillUpdateTarget = {
  skillName: typeof ALICORN_LINEAR_SKILL_NAME | typeof LINEAR_TICKETS_SKILL_NAME
  command: string
}
...
export function getLinearAgentSkillUpdateTarget(
  ...
  const canonicalSkillInstalled = hasInstalledAgentSkill(skills, ALICORN_LINEAR_SKILL_NAME, {
  ...
  return !installed || canonicalSkillInstalled || !legacySkillInstalled
    ? { skillName: ALICORN_LINEAR_SKILL_NAME, command: ALICORN_LINEAR_SKILL_UPDATE_COMMAND }
