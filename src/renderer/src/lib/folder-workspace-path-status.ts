...
export function getFolderWorkspacePathStatusDescription(
  ...
  if (!isHandledPathStatusReason(status.reason)) {
    return translate(
      ...
      'Alicorn cannot use {{path}}, and this version does not recognize the reason. Update Alicorn to see the details.',
      { path: status.path }
    )
  }
  switch (status.reason) {
    case 'missing':
      return translate(
        ...
        'Alicorn cannot find {{path}}. Remove and re-import this folder workspace.',
        { path: status.path }
      )
    case 'not-directory':
      return translate(
    ...
    case 'ambiguous-connection':
      return translate(
        ...
        'Alicorn cannot tell which SSH connection owns this folder scope.'
      )
    case undefined:
    case 'unavailable':
      return translate(
        'auto.lib.folderWorkspacePathStatus.description.unavailable',
        'Alicorn cannot verify this folder right now. Check the runtime or SSH connection and try again.'
      )
  }
}
  ...
  if (message.startsWith('folder_workspace_path_missing:')) {
    return {
      ...
      description: translate(
        ...
        'Alicorn cannot find {{path}}. Remove and re-import the folder.',
        { path }
      )
    }
  }
