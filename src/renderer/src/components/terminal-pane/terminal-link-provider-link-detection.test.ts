...
describe('createFilePathLinkProvider range bounds', () => {
  ...
  it('shows the Alicorn plus default-app hint for local file link hover', async () => {
  ...
  it('shows the Alicorn hint for SSH file link hover', async () => {
    setPlatform('Macintosh')
    ...
    expect(linkTooltip.textContent).toBe(
      '/repo/CLAUDE.md (Click for actions or ⌘+click to open in Alicorn)'
    )
  })
