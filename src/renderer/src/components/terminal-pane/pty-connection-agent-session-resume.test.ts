  ...
  it('uses sleeping-record launch config for pane cold restore after settings change', async () => {
    ...
    const launchConfig = {
      ...
      agentEnv: {
        ...
        ALICORN_PANE_KEY: 'wrong-pane',
        ALICORN_TAB_ID: 'wrong-tab',
        ALICORN_WORKTREE_ID: 'wrong-worktree',
        ALICORN_WORKSPACE_ID: 'wrong-workspace'
      }
    }
    mockStoreState = {
      ...mockStoreState,
    ...
    expect(transport.connect).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          ...
          ALICORN_AGENT_LAUNCH_TOKEN: launchToken
        })
      })
    )
