...
export function getTerminalOrcaFileOpenHint(showActions = true): string {
  ...
  return isMacPlatform()
    ? `${prefix}⌘+click to open in Alicorn`
    : `${prefix}Ctrl+click to open in Alicorn`
}
...
// Why: remote owners advertise Alicorn only when their existing browser route is eligible.
...
export function getTerminalUrlOpenHint(options: TerminalUrlOpenHintOptions = {}): string {
  ...
  if (invertsToOrca) {
    return isMacPlatform()
      ? `${prefix}⌘+click to open, or ⇧⌘+click to open in Alicorn`
      : `${prefix}Ctrl+click to open, or Shift+Ctrl+click to open in Alicorn`
  }
  return isMacPlatform()
...
export function getTerminalUrlOrcaBrowserHint(): string {
  return isMacPlatform() ? '⇧⌘+click to open in Alicorn' : 'Shift+Ctrl+click to open in Alicorn'
}
