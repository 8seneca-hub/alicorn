...
beforeEach(() => {
  ...
  environmentMocks.resolveFooter.mockResolvedValue(
    ['---', 'Alicorn: 1.4.178-rc.2', 'OS: darwin 25.0.0 (arm64)', 'Shell: /bin/zsh'].join('\n')
  )
})
...
describe('humanizeTerminalError', () => {
  it('replaces the pane-owner-unverified code with actionable copy', () => {
    ...
    expect(humanized).toContain('Alicorn left the saved session unchanged')
  ...
  it('humanizes an owner marker without classifying mixed errors as safe warnings', () => {
    ...
    expect(mixed).toContain("Alicorn couldn't verify this terminal's owner.")
...
describe('isExplainedTerminalError', () => {
  ...
  it('keeps the issue link for errors Alicorn cannot explain', () => {
...
describe('shouldOfferDaemonRestart', () => {
  it('matches stale daemon node-pty install failures', () => {
    expect(
      shouldOfferDaemonRestart(
        "Daemon's node-pty install is gone (worktree deleted?). Restart Alicorn. node-pty: posix_spawn failed: ENOENT (errno 2, No such file or directory) - helper='/Applications/Alicorn.app/Contents/Resources/app.asar.unpacked/node_modules/node-pty/build/Release/spawn-helper'"
      )
    ).toBe(true)
  })
  ...
  it('matches stale daemon cwd failures', () => {
    expect(
      shouldOfferDaemonRestart(
        "Daemon's working directory is gone (worktree deleted?). Restart Alicorn. node-pty: daemon_cwd failed: ENOENT (errno 2, No such file or directory) - cwd='<unavailable>'"
      )
    ).toBe(true)
  })
...
describe('TerminalErrorToast environment footer', () => {
  it('appends client environment details to local errors', async () => {
    ...
    await waitFor(() => expect(view.container.textContent).toContain('Alicorn: 1.4.178-rc.2'))
  })
  ...
  it('renders owner-unverified as a warning without an issue link', () => {
    ...
    expect(toast?.textContent).toContain('Alicorn left the saved session unchanged')
