...
describe('HeroFlow height', () => {
  ...
  it('shows Relay mint failure with no QR and the beta note', () => {
    ...
    expect(screen.getByText('Alicorn Relay is in beta.')).toBeInTheDocument()
  })
  ...
  it('does not offer a futile retry when Relay is unavailable on the desktop', () => {
    renderFlow(1, {
      relayMintFailure: {
        ...
        message: 'Alicorn Relay is not available on this desktop'
      }
    })
    expect(screen.getByRole('alert')).toHaveTextContent(
      'Alicorn Relay isn’t available on this desktop'
    )
  ...
  it('demotes the network address picker to a disclosure on Alicorn Relay', async () => {
    const props: React.ComponentProps<typeof MobileHeroPairingStep> = {
      pairQrDataUrl: null,
      pairingUrl: null,
      pairingQrError: false,
  ...
  it('keeps a custom address visible on Alicorn Relay', () => {
