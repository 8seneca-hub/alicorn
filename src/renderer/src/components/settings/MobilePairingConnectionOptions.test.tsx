...
describe('MobilePairingConnectionOptions', () => {
  ...
  it('shows Sign in directly under Alicorn Relay, above LAN', async () => {
    const user = userEvent.setup()
    const onChange = vi.fn()
    render(<MobilePairingConnectionOptions value="automatic" onChange={onChange} />)

    const relay = screen.getByRole('radio', { name: /Alicorn Relay/i })
  ...
  it('moves selection with the arrow keys as a radiogroup', async () => {
    ...
    screen.getByRole('radio', { name: /Alicorn Relay/i }).focus()
  ...
  it('selects a path from the compact list', async () => {
    ...
    await user.click(screen.getByRole('radio', { name: /Alicorn Relay/i }))
    expect(onChange).toHaveBeenCalledWith('automatic')
  })
  ...
  it('shows relay status when signed in on Alicorn Relay', async () => {
    mocks.state = {
      orcaProfileAuthStatus: {
        activeProfileId: 'profile-1',
        configured: true,
