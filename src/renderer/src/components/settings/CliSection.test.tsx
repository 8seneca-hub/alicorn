...
describe('CliSection project runtime defaults', () => {
  it('exposes freshness only for a resolved local host runtime', () => {
    ...
    expect(capturedPanel.props?.freshnessSkillName).toBe('alicorn-cli')
  ...
  it('renders an inline unknown PATH state without offering a mutation', async () => {
    const getInstallStatus = vi.fn().mockResolvedValue({
      platform: 'win32',
      commandName: 'orca',
      commandPath: 'C:\\Program Files\\Alicorn\\resources\\bin\\orca.exe',
      pathDirectory: 'C:\\Program Files\\Alicorn\\resources\\bin',
      pathConfigured: null,
      launcherPath: 'C:\\Program Files\\Alicorn\\resources\\bin\\orca.exe',
      installMethod: 'wrapper',
      supported: true,
      state: 'installed',
      currentTarget: 'C:\\Program Files\\Alicorn\\resources\\bin\\orca.exe',
