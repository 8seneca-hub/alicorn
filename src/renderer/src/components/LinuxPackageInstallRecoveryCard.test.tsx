...
const COPIED_NOTE =
  `Command copied. Quit Alicorn, run it in a system terminal to install ${PACKAGE_FILE_NAME}, ` +
  'then reopen Alicorn.'
const INSTRUCTIONS = {
  ok: true as const,
...
describe('LinuxPackageInstallRecoveryCard copy', () => {
  it('leads with the manual-install copy and recovery actions', () => {
    ...
    expect(
      screen.getByText(/a system terminal on the computer where Alicorn is installed/)
    ).toBeTruthy()
    expect(screen.getByText(/Copy the command, quit Alicorn/)).toBeTruthy()
...
describe('LinuxPackageInstallRecoveryCard details', () => {
  ...
  it('scopes the trust note to the moment the command is built', () => {
    ...
    expect(detail.textContent).toContain(
      'The system package itself is not signature-checked, and Alicorn cannot vouch for the file ' +
        'after that point.'
    )
  })
...
describe('LinuxPackageInstallRecoveryCard without a usable command', () => {
  it('promotes Show Package and keeps the official-release fallback', async () => {
    ...
    expect(screen.getByText(/Quit Alicorn before finishing the update/)).toBeTruthy()
