...
const repos = [repo('r1', 'Alicorn'), repo('r2', 'Alicorn (builder)', 'ssh-1'), repo('r3', 'Solo')]
const projects = [project('p1', 'Alicorn')]
...
describe('buildPaletteFilterModel', () => {
  ...
  it('sorts project rows by workspace count then label', () => {
    ...
    // Alicorn has 2 workspaces, Solo has 1 — popularity beats alpha.
    expect(model.projects.map((option) => option.label)).toEqual(['Alicorn', 'Solo'])
  })
