...
function humanizePermission(permission: string): string {
  switch (permission) {
    ...
    case 'openExternal':
      return 'permission to open a link outside Alicorn'
    case 'fileSystem':
...
export function formatPermissionNotice(event: BrowserPermissionDeniedEvent): string {
  ...
  return `${target} asked for ${humanizePermission(event.permission)}, and Alicorn denied it.`
}
...
export function formatPopupNotice(event: BrowserPopupEvent): string {
  ...
  if (event.action === 'opened-in-orca') {
    return `${target} opened a new page in Alicorn.`
  }
  if (event.action === 'opened-external') {
    return `${target} opened a new window in your default browser.`
  }
  return `${target} tried to open a popup Alicorn does not support here.`
}
  ...
  if (isChromiumCertificateErrorCode(loadError.code)) {
    ...
    if (loadError.code === -202) {
      return translate(
        ...
        "Alicorn doesn't trust the authority that issued the certificate for {{value0}}.",
        { value0: host }
      )
    }
    return translate(
      'browser.loadFailure.certificateVerificationFailed',
      "Alicorn couldn't verify the certificate for {{value0}}.",
      { value0: host }
    )
  }
