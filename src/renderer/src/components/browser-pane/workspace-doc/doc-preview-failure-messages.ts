...
export function docPreviewFailureDetail(reason: DocPreviewFileFailureReason | null): string {
  ...
  return translate(
    ...
    'Alicorn could not read this file from the workspace.'
  )
}
...
export function docPreviewAssetNotice(failures: DocPreviewFileFailure[]): string | null {
  ...
  return translate(
    ...
    'Alicorn could not read {{path}} from the workspace.',
    { path: first.relativePath }
  )
}
 ...
 * row is Alicorn's chrome. Constant text is also what makes repeated attempts unspammable — the
