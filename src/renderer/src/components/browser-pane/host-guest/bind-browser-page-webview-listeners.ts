...
export function bindBrowserPageWebviewListeners({
  ...
  if (needsInitialNavigation) {
    ...
    const initialUrl =
      normalizeBrowserNavigationUrl(initialBrowserUrlRef.current) ?? ALICORN_BROWSER_BLANK_URL
    trackNextLoadingEventRef.current = initialUrl !== ALICORN_BROWSER_BLANK_URL
