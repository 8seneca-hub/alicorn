...
describe('browser-focus', () => {
  ...
  it('requestBrowserFocus queues and dispatches the focus event', () => {
    ...
    window.addEventListener(ALICORN_BROWSER_FOCUS_REQUEST_EVENT, onFocusRequest)
    requestBrowserFocus(detail)
    window.removeEventListener(ALICORN_BROWSER_FOCUS_REQUEST_EVENT, onFocusRequest)
