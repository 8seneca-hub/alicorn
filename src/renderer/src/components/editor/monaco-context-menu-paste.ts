...
// reads the clipboard with navigator.clipboard.readText(). Alicorn runs the renderer with
...
// (execCommand('copy') from a user gesture) both work. We route the read through Alicorn's
 ...
 * bridge); otherwise performs the paste through Alicorn's IPC clipboard read and
