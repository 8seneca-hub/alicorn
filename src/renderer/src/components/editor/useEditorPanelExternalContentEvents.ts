...
export function useEditorPanelExternalContentEvents({
  ...
  useEffect(() => {
    ...
    window.addEventListener(ALICORN_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, handler as EventListener)
    return () =>
  ...
  useEffect(() => {
    ...
    window.addEventListener(ALICORN_EDITOR_FILE_SAVED_EVENT, handler as EventListener)
    return () =>
      window.removeEventListener(ALICORN_EDITOR_FILE_SAVED_EVENT, handler as EventListener)
