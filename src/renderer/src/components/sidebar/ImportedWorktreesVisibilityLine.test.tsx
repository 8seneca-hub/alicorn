...
describe('ImportedWorktreesVisibilityLine', () => {
  it('renders the compact repo-group line with expand and dismiss actions', () => {
    ...
    expect(markup).not.toContain('Alicorn found 4 worktrees')
    ...
    const groups = groupWorktreesByParentPath([
      {
        ...
        path: 'C:\\Repos\\Alicorn\\FeatureX'
      }
    ])

    expect(groups).toMatchObject([{ path: 'C:/Repos/Alicorn' }])
    expect(groups[0]?.path).not.toBe('C:\\Repos\\Alicorn')
  })
