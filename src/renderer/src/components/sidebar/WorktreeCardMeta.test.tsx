...
describe('WorktreeCardDetailsHover', () => {
  ...
  it('puts unlink behind the first PR actions menu and keeps GitHub last', () => {
    ...
    const openInOrcaIndex = markup.indexOf('aria-label="Open in Alicorn"')
    ...
    expect(markup).toContain(
      'Alicorn will hide PR #456 details for this workspace. The PR and branch on GitHub won’t be changed.'
    )
    expect(markup).toContain('Open in Alicorn browser')
    expect(moreActionsIndex).toBeLessThan(openInOrcaIndex)
    expect(openInOrcaIndex).toBeLessThan(viewOnGitHubIndex)
    expect(markup.indexOf('Open in Alicorn browser')).toBeLessThan(markup.indexOf('Copy link'))
  ...
  it('puts issue copy menu before edit and open actions and keeps GitHub last', () => {
    ...
    expect(markup).toContain('Open in Alicorn browser')
    expect(markup.indexOf('Open in Alicorn browser')).toBeLessThan(copyLinkIndex)
  ...
  it('labels GitLab unlink actions with MR terminology', () => {
    ...
    expect(markup).toContain(
      'Alicorn will hide MR !77 details for this workspace. The MR and branch on GitLab won’t be changed.'
    )
  ...
  it('hides the embedded-browser action when a linked review has no URL', () => {
    ...
    expect(markup).not.toContain('Open in Alicorn browser')
  })
