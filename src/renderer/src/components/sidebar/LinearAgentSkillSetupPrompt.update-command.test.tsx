...
function discoveredSkill(overrides: Partial<DiscoveredSkill>): DiscoveredSkill {
  return {
    ...
    directoryPath: '/Users/test/.agents/skills/alicorn-linear',
    skillFilePath: '/Users/test/.agents/skills/alicorn-linear/SKILL.md',
    installed: true,
    updatedAt: null,
    ...overrides
  }
...
describe('LinearAgentSkillSetupPrompt update command', () => {
  ...
  it('uses the canonical update command when the canonical Linear skill is installed', async () => {
    mocks.skillState.skills = [discoveredSkill({ name: 'alicorn-linear' })]

    await renderPrompt()

    expect(mocks.panelProps.at(-1)).toEqual(
      expect.objectContaining({ installedCommand: 'npx skills update alicorn-linear --global' })
    )
  })
  ...
  it('prefers the canonical update command when both Linear skill names are installed', async () => {
    mocks.skillState.skills = [discoveredSkill({ name: 'alicorn-linear' }), legacyLinearSkillPath()]
