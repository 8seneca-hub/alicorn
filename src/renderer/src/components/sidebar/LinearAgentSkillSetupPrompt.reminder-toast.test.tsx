...
function cliStatus(overrides: Partial<CliInstallStatus>): CliInstallStatus {
  return {
    ...
    pathConfigured: true,
    launcherPath: '/Applications/Alicorn.app/Contents/MacOS/Alicorn',
    installMethod: 'symlink',
    supported: true,
    state: 'installed',
    currentTarget: '/Applications/Alicorn.app/Contents/MacOS/Alicorn',
...
describe('LinearAgentSkillSetupPrompt reminder toast', () => {
  ...
  it('shows a warning toast on a later modal-only activation after a casual close', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      'Alicorn CLI and Linear skill are missing',
      expect.objectContaining({
        id: 'linear-agent-skill-setup-orca.linearTicketsSkill.setupDismissed.host',
        description:
          'Install the Alicorn CLI and the Linear skill to enable your agents to read and edit Linear tasks.',
        action: {
          label: 'Set up',
  ...
  it('does not repeat the Alicorn CLI in CLI-only reminder toast copy', async () => {
    mocks.skillState.installed = true
    await snoozeInitialModal({ linked: true, remote: false, surface: 'modal' })
    await renderPrompt({ linked: true, remote: false, surface: 'modal' })

    expect(toast.warning).toHaveBeenCalledWith(
      'Alicorn CLI is missing',
      expect.objectContaining({
        description: 'Install the Alicorn CLI to enable your agents to read and edit Linear tasks.'
      })
    )
  })
  ...
  it('keeps remote setup nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Alicorn CLI and the Linear skill to enable your agents to read and edit Linear tasks. Remote agent environments may need their own setup.'
      })
    )
  })
  ...
  it('keeps WSL target nuance in reminder toast copy', async () => {
    ...
    expect(toast.warning).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        description:
          'Install the Alicorn CLI and the Linear skill to enable your agents to read and edit Linear tasks. This setup runs in the selected WSL agent runtime.'
      })
    )
  })
