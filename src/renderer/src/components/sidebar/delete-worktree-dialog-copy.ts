  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Alicorn. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Alicorn. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Alicorn workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Alicorn. The project folder on disk will not be deleted.'
