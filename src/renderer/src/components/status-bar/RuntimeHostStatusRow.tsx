  ...
  switch (state) {
    ...
    case 'runtime-unavailable':
      return translate(
        ...
        'SSH transport is connected, but the Alicorn runtime is unavailable'
      )
    case 'checking':
      return translate(
        'auto.components.status.bar.RuntimeHostStatusRow.checking_host',
        'Alicorn is checking whether this host is reachable'
      )
    case 'reconnecting':
      return translate(
        'auto.components.status.bar.RuntimeHostStatusRow.restoring_connection',
        'Alicorn is trying to restore the connection'
      )
    case 'disconnected':
      return translate(
        'auto.components.status.bar.RuntimeHostStatusRow.host_unreachable',
        'Alicorn isn’t reachable on this host'
      )
  }
}
...
function runtimeFailureExplanation(state: RuntimeHostConnectionState): string | null {
  ...
  if (state === 'runtime-unavailable') {
    return translate(
      ...
      'The remote host may still be running; only the Alicorn runtime connection is unavailable.'
    )
  }
  return translate(
    'auto.components.status.bar.RuntimeHostStatusRow.contact_note',
    'The host may still be running; only the Alicorn connection is unavailable.'
  )
}
