...
function copiedNote(packageFileName: string): string {
  return translate(
    ...
    'Command copied. Quit Alicorn, run it in a system terminal to install {{value0}}, then reopen Alicorn.',
    {
...
export function LinuxPackageInstallRecoveryCard({
  ...
  const SUMMARY = translate(
    ...
    'Alicorn downloaded the system package. Quit Alicorn before finishing the update from a terminal.'
  )
  const EXPLAINER = translate(
    'auto.components.LinuxPackageInstallRecoveryCard.82c6dbea00',
    'Copy the command, quit Alicorn, and run it in a system terminal on the computer where Alicorn is installed. Reopen Alicorn after it finishes.'
  )
  ...
  const TRUST_NOTE = translate(
    ...
    'Alicorn checks the downloaded file against the release metadata at the moment it builds this command. The system package itself is not signature-checked, and Alicorn cannot vouch for the file after that point.'
  )
