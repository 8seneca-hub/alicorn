 ...
 * wrong in a way the update cannot fix — an edited copy, one Alicorn could not read.
 *
...
export function groupSkillFreshness(
  ...
  for (const [name, entries] of byName) {
    ...
    // row would assert Alicorn considered an update it never could. Locations below are still
