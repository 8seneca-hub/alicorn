...
describe('checks panel hosted review modifier hint destination', () => {
  it('names the system browser when a plain click already opens in Alicorn', () => {
    expect(resolveChecksPanelHostedReviewModifierDestination({ openLinksInApp: true }, true)).toBe(
      'system-browser'
    )
  })

  // Why: inverting is inert while links already open in Alicorn — both meanings of the
  // modifier land on the system browser, so checking inverts first would misname it.
  it('names the system browser when inverting is on and links already open in Alicorn', () => {
    expect(
      resolveChecksPanelHostedReviewModifierDestination(
  ...
  // and the only way to reach Alicorn from this button is undiscoverable.
  it('names Alicorn when inverting is on and links open externally', () => {
    expect(
      resolveChecksPanelHostedReviewModifierDestination(
  ...
  // Why: openHttpLink refuses to route a remote-owned link into Alicorn, and openLinksInApp
  ...
  // not suppress a hint for a click that still reaches Alicorn.
  ...
  // Why: openHttpLink gates routing to Alicorn on a worktree id, so without one the
