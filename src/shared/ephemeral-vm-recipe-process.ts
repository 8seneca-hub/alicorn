...
function buildRecipeEnv(
  ...
  return {
    ...process.env,
    ...env,
    ALICORN_VM_MODE: mode,
    ALICORN_VM_INSTANCE_ID: context.instanceId ?? '',
    ALICORN_RECIPE_ID: context.recipeId,
    ALICORN_PROJECT_ID: context.projectId ?? '',
    ALICORN_WORKSPACE_ID: context.workspaceId ?? '',
    ALICORN_WORKSPACE_NAME: context.workspaceName ?? '',
    ALICORN_REPO_PATH: context.repoPath,
    ALICORN_REPO_URL: context.repoUrl ?? '',
    ALICORN_REPO_BRANCH: context.branch ?? '',
    ALICORN_REPO_REF: context.ref ?? '',
    ALICORN_REPO_REF_HEAD: context.expectedRefHead ?? '',
    ALICORN_RECIPE_RESULT_SCHEMA_VERSION: String(resultSchemaVersion),
    ALICORN_VERSION: context.orcaVersion ?? ''
  }
}
