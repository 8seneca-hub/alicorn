...
export function buildWslInteractiveLoginShellCommand(): string {
  return [
    ...
    // resolved root and WSLENV /p-translates it. The ALICORN_USER_DATA_PATH branch
    // stays as the fallback for an older host that exports only that.
    'if [ -n "${ALICORN_SHELL_READY_ROOT:-}" ]; then',
    '  _orca_shell_ready_root="${ALICORN_SHELL_READY_ROOT%/}"',
    'elif [ -n "${ALICORN_USER_DATA_PATH:-}" ]; then',
    '  _orca_shell_ready_root="${ALICORN_USER_DATA_PATH%/}/shell-ready"',
    'fi',
    '_orca_wsl_shell_name=$(basename "$_orca_wsl_shell" | tr "[:upper:]" "[:lower:]")',
    'case "$_orca_wsl_shell_name" in',
    '  bash)',
