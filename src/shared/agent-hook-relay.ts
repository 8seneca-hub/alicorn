...
// the relay sends to Alicorn. Consumed by `src/relay/agent-hook-server.ts` (which
...
// between Alicorn's main process and the remote relay.
//
// Invariants both ends rely on:
// - The relay normalizes; Alicorn routes. The envelope's `payload` field has
//   already been through `normalizeHookPayload` (which calls
//   `parseAgentStatusPayload` → `normalizeAgentStatusObject`) on the relay
//   side. Alicorn's `ingestRemote` re-runs the canonical payload normalizer at
...
// - The wire `connectionId` is **always `null`**: a `connectionId` is Alicorn's
//   local handle on an `ssh2` connection, not a wire identity. Alicorn stamps the
...
//   POST body so Alicorn's warn-once protocol diagnostics still fire. The relay
...
// with the same value so Alicorn can attribute the event back to the right CLI.
...
/** Wire envelope for a single hook event flowing relay → Alicorn. */
export type AgentHookRelayEnvelope = {
  source: AgentHookSource
  paneKey: string
  /** Ephemeral Alicorn launch identity stamped into the PTY env for this process. */
  launchToken?: string
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know Alicorn's local connectionId. */
  connectionId: null
  /** Preserved from the relay-side normalized hook event so Alicorn can
  ...
  /** True when the relay is replaying its cache after Alicorn reconnects. */
  isReplay?: boolean
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets Alicorn's warn-once
   ...
   *  Alicorn's `ingestRemote` validates it again at the SSH trust boundary. */
...
/**
 * Re-attaches shed fields from Alicorn's cached payload for this pane, so a transport-level
...
/** JSON-RPC request method Alicorn issues after `--connect` reattach to ask the
 *  relay to replay its per-paneKey last-payload cache. Pull, not push: a relay
 *  that pushed on `setWrite` can emit before Alicorn has wired its `agent.hook`
...
/** JSON-RPC request method Alicorn issues at session-ready to ship the
...
export type AgentHookInstallManagedHooksParams = {
  /** SHA-256 fingerprint of the server key negotiated by Alicorn's SSH transport. */
  hostKeyFingerprint?: string
  /** Positively detected and enabled agents allowed to mutate remote config. */
  agents: readonly AgentHookTarget[]
}

/** Feature-flag env var. Read once at process start by Alicorn and the relay.
