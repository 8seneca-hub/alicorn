...
/**
 * `'posix'` covers every Unix shell Alicorn can type into, fish included — not
 ...
 * is deliberately no fish member: a dialect Alicorn has to detect is a dialect it
...
export function tokenizeStartupCommand(
  ...
  // Why one Unix parse: the input is a string the user typed into an Alicorn
...
/**
 * Quotes one argument so the SAME text is literal in every Unix shell Alicorn can
 ...
 * of them, and Alicorn never has to know which shell will read the line.
 *
 ...
 *
 * Why not a helper function defined by Alicorn's shell wrappers: Alicorn only wraps
 ...
 * shells Alicorn never spawned — in all of those a helper would be `command not
