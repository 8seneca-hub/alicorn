export const ALICORN_RENDERER_UNLOAD_PREVENTED_EVENT = 'orca:renderer-unload-prevented'
export const ALICORN_RENDERER_SHUTDOWN_CHECKPOINT_FAILED_EVENT =
  'orca:renderer-shutdown-checkpoint-failed'
export const ALICORN_RENDERER_SHUTDOWN_CHECKPOINT_ABORTED_EVENT =
...
export const ALICORN_SHUTDOWN_CHECKPOINT_FAILURE_REASON_ATTRIBUTE =
...
export function publishShutdownCheckpointFailureReason(reason: string): void {
  try {
    globalThis.document?.documentElement?.setAttribute(
      ALICORN_SHUTDOWN_CHECKPOINT_FAILURE_REASON_ATTRIBUTE,
      reason
    )
  } catch {
...
export function consumeShutdownCheckpointFailureReason(): string | null {
  try {
    ...
    const reason = root?.getAttribute(ALICORN_SHUTDOWN_CHECKPOINT_FAILURE_REASON_ATTRIBUTE)
    if (reason) {
      root?.removeAttribute(ALICORN_SHUTDOWN_CHECKPOINT_FAILURE_REASON_ATTRIBUTE)
    }
    return reason || null
  } catch {
    return null
