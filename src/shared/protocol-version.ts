...
// Why: declares the Alicorn runtime RPC compatibility contract. Desktop,
...
// Why negotiated and not additive: `compatibilityCliCommand` is a zod enum, and an out-of-domain
// *value* on a known key fails validation outright — unlike an unknown key, which is stripped. A
// client that sent `alicorn` to a host predating this capability would fail the whole RPC rather
// than degrade, so the client sends the post-rebrand spelling only once the host advertises this.
export const ORCHESTRATION_ALICORN_CLI_COMMAND_RUNTIME_CAPABILITY =
  'orchestration.alicorn-cli-command.v1' as const
...
export const JIRA_USER_FIELDS_UPDATE_REQUIRED_MESSAGE =
  'Creating Jira issues with user fields requires a newer Alicorn server. Update the server and try again.'
...
export const FILE_MUTATION_OWNERSHIP_UPDATE_REQUIRED_MESSAGE =
  'Remote file changes require a newer Alicorn server. Update the HUB and try again.'
...
export const GITHUB_MARK_PR_READY_UPDATE_REQUIRED_MESSAGE =
  'Marking a pull request ready requires a newer Alicorn server. Update the server and try again.'
...
export const GITLAB_READY_FOR_REVIEW_UPDATE_REQUIRED_MESSAGE =
  'Marking a merge request ready requires a newer Alicorn server. Update the server and try again.'
...
export const AUTOMATION_LIST_HOST_SCOPE_UPDATE_REQUIRED_MESSAGE =
  'Filtering automations by host requires a newer Alicorn server. Update the HUB and try again.'
...
export const AUTOMATION_OWNER_FENCING_UPDATE_REQUIRED_MESSAGE =
  'Editing automations on this host requires a newer Alicorn server. Update the HUB and try again.'
