...
export function verifyRemotePairingRuntimeStatus(
  ...
  if (status.deviceScope === 'mobile') {
    return {
      ...
      message: 'This link grants mobile-only access. Generate a link for another Alicorn client.'
    }
  }
  const versionFields = [
    ...
    return {
      ...
      message:
        compatibility.reason === 'client-too-old'
          ? 'Update this Alicorn client before adding the remote host.'
          : 'Update Alicorn on the remote host before adding it.'
    }
  }
