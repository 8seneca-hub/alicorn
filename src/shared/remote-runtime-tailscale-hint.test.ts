...
const UNREACHABLE = 'Could not connect to the remote Alicorn runtime.'
...
describe('withRemoteRuntimeTailscaleHint', () => {
  ...
  it('covers the close and timeout failure variants', () => {
    ...
    expect(
      withRemoteRuntimeTailscaleHint(
        'Timed out while connecting to the remote Alicorn runtime.',
        'wss://host.ts.net'
      )
  ...
  it('leaves non-connectivity errors untouched', () => {
    const auth = 'Remote Alicorn runtime rejected the pairing token.'
