 ...
 * already delivers the configured root as `ALICORN_WORKSPACE_ROOT`; read it before falling back.
 *
...
export function resolveRelaySpawnCwd(args: {
  ...
  const workspaceId =
    trimmedString(args.worktreeId) ?? trimmedString(args.env?.ALICORN_WORKSPACE_ID)
  ...
  const workspaceRoot = trimmedString(args.env?.ALICORN_WORKSPACE_ROOT)
