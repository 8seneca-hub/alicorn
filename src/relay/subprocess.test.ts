...
describe('Subprocess: Relay entry point', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      relay = spawn(
        ...
        { ...process.env, ALICORN_RELAY_EMPTY_STARTUP_GRACE_MS: '100' }
      )
      await relay.sentinelReceived
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      relay = spawn(['--detached', '--grace-time', '1', '--sock-path', sockPath], {
        ...process.env,
        ALICORN_RELAY_EMPTY_STARTUP_GRACE_MS: '500'
      })
      await relay.sentinelReceived
  ...
  function spawnIdleGraceDaemon(
    ...
    const daemon = spawnRelayEntry(
      ...
      { ...process.env, ALICORN_RELAY_IDLE_GRACE_MS: idleGraceMs }
    )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      relay = spawn(['--detached', '--grace-time', '0', '--sock-path', sockPath], {
        ...process.env,
        ALICORN_RELAY_IDLE_GRACE_MS: '200'
      })
      await relay.sentinelReceived
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      relay = spawnRelayEntry(
        ...
        { ...process.env, ALICORN_RELAY_IDLE_GRACE_MS: '200' }
      )
      await relay.sentinelReceived
