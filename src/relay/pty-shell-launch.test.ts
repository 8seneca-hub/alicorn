...
describe('getRelayShellLaunchConfig', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const config = getRelayShellLaunchConfig('/bin/zsh', {
        HOME: homeDir,
        ALICORN_OPENCODE_CONFIG_DIR: '/tmp/orca-opencode-overlay'
      })
      ...
      // Why no ALICORN_USER_ZDOTDIR: the relay used to republish the inherited
      ...
      expect(zshenv).not.toContain('ALICORN_USER_ZDOTDIR')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      // bridge sets ALICORN_REMOTE_CLI_BIN_DIR, which was already enough to wrap.
      ...
      env.ALICORN_HISTFILE = join(homeDir, 'orca-history', 'zsh_history')

      const config = getRelayShellLaunchConfig('/bin/zsh', env)

      expect(config.env.ZDOTDIR).toBe(join(homeDir, '.orca-relay', 'shell-ready', 'zsh'))
      expect(config.env.ALICORN_SHELL_FEATURES).toBe('overlay,history,markers')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const config = getRelayShellLaunchConfig('/bin/zsh', {
        HOME: homeDir,
        ALICORN_MIMOCODE_HOME: '/tmp/orca-mimocode-overlay'
      })
      const zshRoot = join(homeDir, '.orca-relay', 'shell-ready', 'zsh')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env).toEqual({ ALICORN_SHELL_FEATURES: '' })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.ALICORN_SHELL_FEATURES).toContain('ready')
      ...
      expect(zshenv).toContain('builtin unset ALICORN_SHELL_FEATURES')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(bashRc).toContain('builtin unset ALICORN_SHELL_FEATURES')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.ZDOTDIR).toBe(join(homeDir, '.orca-relay', 'shell-ready', 'zsh'))
      expect(config.env.ALICORN_SHELL_FEATURES).toContain('identity')
      ...
      expect(config.env.ALICORN_SHELL_FEATURES).not.toContain('ready')
