  ...
  private buildPluginEnvironment(context: Parameters<PtyEnvAugmenter>[0]): Record<string, string> {
    ...
    if (this.pluginOverlay.hasOpenCodeSource()) {
      ...
      if (dir) {
        ...
        env.ALICORN_OPENCODE_CONFIG_DIR = dir
        if (sourceDir) {
          env.ALICORN_OPENCODE_SOURCE_CONFIG_DIR = sourceDir
        }
      }
    }
    ...
    if (kind === 'pi') {
      ...
      if (result?.sourceAgentDir) {
        env.ALICORN_PI_SOURCE_AGENT_DIR = result.sourceAgentDir
      }
    }
    if (kind === 'omp' || !hasLaunchCommand) {
      const sourceDir =
        kind === 'omp'
          ...
          : context.env.ALICORN_OMP_SOURCE_AGENT_DIR
      ...
      if (result?.statusExtensionPath) {
        env.ALICORN_OMP_STATUS_EXTENSION = result.statusExtensionPath
      }
      if (result?.sourceAgentDir) {
        env.ALICORN_OMP_SOURCE_AGENT_DIR = result.sourceAgentDir
      }
    }
    if (kind === 'prime-agent') {
      ...
      if (result?.sourceAgentDir) {
        env.ALICORN_PRIME_AGENT_SOURCE_AGENT_DIR = result.sourceAgentDir
      }
    }
    return env
  }
