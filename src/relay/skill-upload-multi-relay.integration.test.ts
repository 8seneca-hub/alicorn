...
beforeAll(async () => {
  const externalEntry = process.env.ALICORN_SKILL_UPLOAD_RELAY_ENTRY
  if (externalEntry) {
...
describe('skill upload ownership across relay processes', () => {
  ...
  it
    .runIf(Boolean(process.env.ALICORN_SKILL_UPLOAD_LEGACY_RELAY_ENTRY))
    ...
    async (order) => {
      ...
      const home = join(root, 'home')
      const environment = { ...process.env, HOME: home, USERPROFILE: home }
      const legacyEntry = resolve(process.env.ALICORN_SKILL_UPLOAD_LEGACY_RELAY_ENTRY!)
      const entries =
        order === 'legacy-first'
          ? [
