...
description: >-
  ...
  `alicorn-linear`; remains available for existing installs.
---

# Linear Tickets (Legacy Name)

This file is a discovery stub, not the usage guide. `linear-tickets` is the legacy bundled
name for `alicorn-linear`; both resolve to the same Linear CLI (`alicorn linear ...`). The full,
version-matched reference is served by the `alicorn` binary itself — kept out of this file on
...
PR/MR links, and triaging issues. The `alicorn-linear` topic serves the same content. Read it
