...
environment reference is served by the `alicorn` binary itself — kept out of this file on
...
an `alicorn vm recipe doctor` failure. Orca is a thin wrapper: you guide, detect, and scaffold;
...
```text
ORCA skills get alicorn-per-workspace-env
```

That prints the complete, version-matched guide for the exact binary that will handle your
next commands — provider setup, base and auth snapshots, `environmentRecipes` in
`orca.yaml`, lifecycle scripts, and `alicorn vm recipe doctor`. Read it first, then run the
...
`ORCA skills get alicorn-per-workspace-env`. Beyond these commands, ask the user rather than
