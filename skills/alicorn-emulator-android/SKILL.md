...
emulator reference is served by the `alicorn` binary itself — kept out of this file on purpose
...
logcat. It is cross-platform (Windows, Linux, macOS) and complements the alicorn-emulator (iOS)
and alicorn-cli skills.
...
```text
ORCA skills get alicorn-emulator-android
```
...
`ORCA skills get alicorn-emulator-android`. Beyond these commands, ask the user rather than
